...
{
    public partial class ReceiptReportParam_WindowController : WindowController
    {
        public ReceiptReportParam_WindowController()
        {
            InitializeComponent();
        ...
        {
            if (e.ObjectType.IsAssignableFrom(typeof(ReceiptReportParam)))
            {
                if (((int)e.Key) == 0)
                {
                    var objectSpace = Application.CreateObjectSpace(typeof(Storage));
                    ReceiptReportParam defaultObject = new ReceiptReportParam();
                    defaultObject.ID = 0;
