...
{
    public class GA_PackingLineProcess
    {
            ...
            List<PackingOverQuantity> overQuantities, bool IsRemain, out List<PackingOverQuantity> outOverQuantites)
        {
            ...
            int SequenceNo = 0;
            var lsStyles = orderDetailForPackings.OrderBy(x => x.LSStyle).Select(x => x.LSStyle).Distinct().ToList();   
            ...
            switch (type)
            {
                case PackingType.SolidSizeSolidColor:
                    if (!IsRemain)
                    {
                        ...
                        {
                            if (totalQuantity > 0 && quantityPackagePercarton > 0)
                            {
                                foreach (var orderDetailForPacking in orderDetailForPackings.Where(x => x.LSStyle == style)
                                    .OrderBy(x => x.SizeSortIndex))
                                {
                                    SequenceNo += 1;

                                    int totalCarton = (int)(orderDetailForPacking.ShipQuantity / (totalQuantity * quantityPackagePercarton));
                                    var netWeight = styleNetWeights
                                            .FirstOrDefault(x => x.Size == orderDetailForPacking.Size &&
                                                            x.CustomerStyle == orderDetailForPacking.CustomerStyle)?.NetWeight ?? 0;
                                    if (totalCarton > 0)
                                    {
                                        var packingLine = new PackingLine()
                                        {
                                            ...
                                            QuantityPerCarton = totalQuantity * quantityPackagePercarton,
                                            QuantitySize = totalQuantity * quantityPackagePercarton,
                                            TotalCarton = (int)(orderDetailForPacking.ShipQuantity / (totalQuantity * quantityPackagePercarton)),
                                            QuantityPerPackage = totalQuantity,
                                            ...
                                            FromNo = CartonNo,
                                            ToNo = CartonNo + (int)(orderDetailForPacking.ShipQuantity / (totalQuantity * quantityPackagePercarton)) - 1,
                                            DeliveryPlace = orderDetailForPacking.DeliveryPlace
                                        };

                                        packingLine.TotalQuantity = packingLine.QuantitySize * packingLine.TotalCarton;
                                        ...
                                        CartonNo += (int)(orderDetailForPacking.ShipQuantity / (totalQuantity * quantityPackagePercarton));
                                    ...
                                    {
                                        packingOverQuantity.Quantity = orderDetailForPacking.ShipQuantity % (totalQuantity * quantityPackagePercarton);
                                    }
                                    else
                                    {
                                        packingOverQuantity = new PackingOverQuantity()
                                        {
                                            Quantity = orderDetailForPacking.ShipQuantity % (totalQuantity * quantityPackagePercarton),
                            ...
                            else if (totalQuantity > 0 && quantityPackagePercarton == 0)
                            {
                                foreach (var orderDetailForPacking in orderDetailForPackings.Where(x => x.LSStyle == style)
                                    .OrderBy(x => x.SizeSortIndex))
                                {
                    ...
                    else
                    {
                        foreach (var style in lsStyles)
                        {
                            ...
                            {
                                foreach (var overQuantity in overQuantities.Where(x => x.ItemStyle.LSStyle == style)
                                    .OrderBy(x => x.SizeSortIndex))
                                {
                                    var netWeight = styleNetWeights
                                            .FirstOrDefault(x => x.Size == overQuantity.Size &&
                                                            x.CustomerStyle == overQuantity.ItemStyle.CustomerStyle)?.NetWeight ?? 0;
                ...
                case PackingType.AssortedSizeSolidColor:
                    if (!IsRemain)
                    {
                        foreach(var style in lsStyles)
                        {
                            ...
                            foreach (var sizePacking in orderDetailForPackings.Where(x => x.LSStyle == style))
                            {
                    ...
                    else
                    {
                        foreach (var style in lsStyles)
                        {
                            ...
                            foreach (var sizePacking in overQuantities.Where(x => x.ItemStyle.LSStyle == style))
                            {
                                var sizeRatio = ratios
                                    .FirstOrDefault(x => x.Size == sizePacking.Size)?.Ratio ?? 0;
                                if (sizeRatio == 0)
