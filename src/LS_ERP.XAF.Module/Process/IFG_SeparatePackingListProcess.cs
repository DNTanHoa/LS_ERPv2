...
namespace LS_ERP.XAF.Module.Process
{
    public class IFG_SeparatePackingListProcess
    {
        public static List<PackingLine> SeparatePacking(List<PackingLine> packingLines,
           ...
           Dictionary<string,decimal> separateCarton, bool isRemain)
        {
            ...
            if (packingLines?.FirstOrDefault().PrePack.Trim() == "Assorted Size - Solid Color")
            {
                foreach(var style in ifgStyles)
                {
                    ...
                    foreach (var orderDetail in style.OrderDetails.OrderBy(x => x.SizeSortIndex))
                    {
                        ...
                        localCartonNo = (int)(!isRemain ? line.FromNo
                                        : line.FromNo + separateCarton[orderDetail.Size]);
                        ...
                        var packingLine = new PackingLine()
                        {
                            ...
                            ToNo = (int)(isRemain ? line.ToNo
                                        : line.FromNo + separateCarton[orderDetail.Size] - 1),
                        ...
                        else
                        {
                            ...
                            if (totalCarton > 0)
                            {
                                var packingLine = new PackingLine()
                                {
                                    ...
                                    FromNo = (int)(!isRemain ? currentPackingLine.FromNo
                                        : currentPackingLine.FromNo + separateCarton[orderDetail.Size]),
                                    ToNo = (int)(isRemain ? currentPackingLine.ToNo
                                        : currentPackingLine.FromNo + separateCarton[orderDetail.Size] - 1),
                                    DeliveryPlace = style.DeliveryPlace
                                };
