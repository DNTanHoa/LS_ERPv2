...
{
    public class ExportInvoiceGA_Processor
    {
        ...
        private static Dictionary<int, string> _dicSizeList;
        ...
        private static void CreateHeaderInvoice(ExcelWorksheet workSheet, Invoice invoice)
        {
            ...
            workSheet.Cells[_maxRow, 2].Value = invoice.PortOfDischarge?.Name?.ToUpper()
                                                + ", " + invoice.CustomerID
                                                + ", " + invoice.PortOfDischarge?.Country?.Code?.ToUpper();
        ...
        private static void FillDataInvoice(ExcelWorksheet workSheet, Invoice invoice)
        {
            ...
            {
                if (packinglist?.PackingLines?.FirstOrDefault()?
                    .PrePack == "Assorted Size - Solid Color")
                    totalCarton += (double)packinglist?.PackingLines?.FirstOrDefault()?.TotalCarton;
                else
                {
                    var firstStyle = packinglist?.ItemStyles?.FirstOrDefault();
                    totalCarton += (double)packinglist?.PackingLines?.
                        Where(x => x.LSStyle == firstStyle.LSStyle)?.Sum(x => x.TotalCarton);
                }    
            }
            ...
            foreach (var invoiceDetail in invoice.InvoiceDetails
                .OrderBy(x => x.CustomerPurchaseOrderNumber + x.CustomerStyle + x.GarmentColorCode))
            {
                _maxRow++;
                _maxRow++;
            ...
            var companyAddress = invoice?.Company?.DisplayAddress?.ToUpper().Split(",").ToList();
            var addr1 = "";
            var addr2 = "";
            SplitAddress(companyAddress, ref addr1, ref addr2);

            _maxRow++;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[DESCRIPTION_OF_GOODS]].Value = addr1;// invoice.Company.DisplayAddress.Substring(0, 57);

            _maxRow++;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[DESCRIPTION_OF_GOODS]].Value = addr2;// invoice.Company.DisplayAddress.Substring(59);
            ...
            _maxRow++;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[QUANTITY] - 1].Value = addr1;//invoice.Company.DisplayAddress.Substring(0, 57)?.ToUpper();

            _maxRow++;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[QUANTITY] - 1].Value = addr2;// invoice.Company.DisplayAddress.Substring(59)?.ToUpper();
        ...
        private static void CreateHeaderDataPL(ExcelWorksheet workSheet, int row, PackingList packingList)
        {
            ...
            if(!_dicPositionColumnPackingLine.ContainsKey(FROM_NO))
                _dicPositionColumnPackingLine.Add(FROM_NO, 1);

            if (!_dicPositionColumnPackingLine.ContainsKey(TO_NO))
            ...
            if (!_dicPositionColumnPackingLine.ContainsKey(CNTS))
            ...
            if (!_dicPositionColumnPackingLine.ContainsKey(DESCRIPTION))
            ...
            if (!_dicPositionColumnPackingLine.ContainsKey(PO_NO))
                _dicPositionColumnPackingLine.Add(PO_NO, column + 2);

            workSheet.Cells[row + 3, column].Value = COLOR;

            if (!_dicPositionColumnPackingLine.ContainsKey(COLOR))
            ...
            if (!_dicPositionColumnPackingLine.ContainsKey(COLOR_NO))
            ...
            if (!_dicPositionColumnPackingLine.ContainsKey(SIZE))
            ...
            var sizeList = GetSizeList(packingList);

            foreach (var order in sizeList)
            {
                string size =order.Value;
                if (!string.IsNullOrEmpty(size))
                {
                    workSheet.Cells[row + 3, column].Value = size;
                    workSheet.Cells[row + 3, column].Style.WrapText = true;

                    if (!_dicPositionColumnPackingLine.ContainsKey(packingList.ID + size))
                        _dicPositionColumnPackingLine.Add(packingList.ID + size, column);
            ...
            if (_dicAlphabel.TryGetValue(column, out colChar))
            {
                ...
                if (!_dicPositionColumnPackingLine.ContainsKey(UNITS_CARTON))
            ...
            if (_dicAlphabel.TryGetValue(column, out colChar))
            {
                ...
                if (!_dicPositionColumnPackingLine.ContainsKey(TOTAL_UNITS))
            ...
            if (_dicAlphabel.TryGetValue(column, out colChar))
            {
                ...
                if (!_dicPositionColumnPackingLine.ContainsKey(GW_CARTON))
            ...
            if (_dicAlphabel.TryGetValue(column, out colChar))
            {
                ...
                if (!_dicPositionColumnPackingLine.ContainsKey(TOTAL_GW))
            ...
            if (_dicAlphabel.TryGetValue(column, out colChar))
            {
                ...
                if (!_dicPositionColumnPackingLine.ContainsKey(NW_CARTON))
            ...
            if (_dicAlphabel.TryGetValue(column, out colChar))
            {
                ...
                if (!_dicPositionColumnPackingLine.ContainsKey(TOTAL_NW))
        ...
        private static void FillDataPL(ExcelWorksheet workSheet, Invoice invoice)
        {
            ...
            foreach (var packinglist in invoice.PackingList
                .OrderBy(x => x?.ItemStyles?.FirstOrDefault()?.PurchaseOrderNumber +
                              x?.ItemStyles?.FirstOrDefault()?.CustomerStyle +
                              x?.ItemStyles?.FirstOrDefault()?.ColorCode))
            {
                ...
                foreach (var data in packinglist.ItemStyles)
                {
                    ...
                    netWeight += (decimal)(sortPackingLines
                        .Where(x => x.LSStyle == data.LSStyle)?.FirstOrDefault()?.NetWeight ?? 0);
                }
                    ...
                    foreach (var packingLine in sortPackingLines)
                    {
                        ...
                        if (firstRow || packingLine.LSStyle == lsStyle)
                        {
                            if (firstRow)
                            {
                                ...
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[packID + packingLine.Size]].Value = packingLine.Quantity;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[packID + packingLine.Size]].Style.Font.Bold = false;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON]].Value = packingLine.QuantityPerCarton * styleCount;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_UNITS]].Value = packinglist.TotalQuantity;
                                ...
                                //var key = stt.ToString("d5") + ";" + itemStyle.LSStyle + ";" + packingLine.Size;
                                ...
                                totUnits += (decimal)packinglist.TotalQuantity;
                ...
                else
                {
                    ...
                    foreach (var packingLine in sortPackingLines)
                    {
                        ...
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[packID + packingLine.Size]].Value =  packingLine.Quantity * styleCount;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON]].Value = packingLine.QuantityPerCarton * styleCount;
                        ...
                        //    var key = stt.ToString("d5") + ";" + style.LSStyle + ";" + packingLine.Size;
                        //    if (_dicTotalQuantitySize.TryGetValue(key, out decimal quantity))
                        //    {
        ...
        private static void SummaryDataPL(ExcelWorksheet workSheet, Invoice invoice)
        {
            ...
            invoice.PackingList.ToList().ForEach(x =>
                {
                    x.ItemStyles.ToList().ForEach(i =>
                    {
                        ...
                        i?.OrderDetails.ToList().ForEach(s =>
                        {
                            var lines = x?.PackingLines?.Where(p => p.LSStyle == i.LSStyle && p.Size == s.Size).ToList();
                            if(lines.Any())
                            {
                                var key = i.LSStyle + ";" + s.Size;
                                if (_dicTotalQuantitySize.TryGetValue(key, out decimal quantity))
                                {
                                    quantity += (decimal)lines.Sum(l => l.QuantitySize * l.TotalCarton);
                                    _dicTotalQuantitySize[key] = quantity;
                                }
                                else
                                {
                                    _dicTotalQuantitySize[key] = (decimal)lines.Sum(l => l.QuantitySize * l.TotalCarton);
                                }
                            }
                            else
                            {
                ...
                .OrderBy(x => x.CustomerPurchaseOrderNumber + x.CustomerStyle + x.GarmentColorCode))
            {
                var itemStyle = itemStyles?.
                    FirstOrDefault(x => x.PurchaseOrderNumber == detail.CustomerPurchaseOrderNumber &&
                                    x.CustomerStyle == detail.CustomerStyle && x.ColorCode == detail.GarmentColorCode);

                var packingList = invoice?.PackingList?.FirstOrDefault(p => p.ItemStyles.Contains(itemStyle));

                var listQuantities = _dicTotalQuantitySize.Where(x => x.Key.Split(";")[0] == itemStyle.LSStyle).ToList();
                var firstSize = true;
                var lastSize = "";

                foreach (var data in listQuantities.OrderBy(l => l.Key))
                {
                    ...
                    if (firstSize)
                    {
                        ...
                        lastSize = listQuantities.OrderByDescending(x => x.Key).FirstOrDefault().Key.Split(";")[1];
                        ...
                        int i = 0;
                        var sizeList = GetSizeList(packingList);
                        //foreach (var order in itemStyle?.OrderDetails.OrderBy(x => x.SizeSortIndex))
                        foreach (var order in sizeList.OrderBy(x => x.Key))
                        {
                            if (!string.IsNullOrEmpty(itemStyle?.OrderDetails
                                ?.FirstOrDefault(o => o.Size == order.Value)?.Size))
                            {
                                string sizeName = order.Value;
                        ...
                        workSheet.Cells[_maxRow + 1, _dicPositionColumnPackingLine[packingList.ID + size]].Value = data.Value;
                        workSheet.Cells[_maxRow + 1, _dicPositionColumnPackingLine[packingList.ID + size]].Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                        workSheet.Cells[_maxRow + 1, _dicPositionColumnPackingLine[packingList.ID + size]].Style.VerticalAlignment = ExcelVerticalAlignment.Center;
            ...
            _maxRow++;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_UNITS]].Value = addr1; // invoice.Company.DisplayAddress.Substring(0, 57)?.ToUpper();
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_UNITS]].Style.Font.Bold = false;

            _maxRow++;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_UNITS]].Value = addr2; // invoice.Company.DisplayAddress.Substring(59)?.ToUpper();
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_UNITS]].Style.Font.Bold = false;
        ...
        private static void CreateHeaderSTDPL(ExcelWorksheet workSheet, Invoice invoice)
        {
            ...
            workSheet.Cells[_maxRow, _colEndSize + 1].Value = "Port of Discharge: " + invoice?.PortOfDischarge?.Name?.ToUpper() + ", " +
                 invoice?.CustomerID?.ToUpper() + ((invoice?.PortOfDischarge?.CountryID ?? 0) != 0 ? (", " + invoice?.PortOfDischarge?.Country?.Code?.ToUpper()) : string.Empty);
        ...
        private static void CreateHeaderDataSTDPL(ExcelWorksheet workSheet, Invoice invoice)
        {
            ...
            {
                var col = _colSize;
                var listSize = GetSizeList(packingList);
                foreach(var data in listSize.OrderBy(x => x.Key))
                {
                    if (!_dicPositionColumnPackingLine.ContainsKey(packingList.ID + data.Value))
                        _dicPositionColumnPackingLine.Add(packingList.ID + data.Value, col);

                    col++;
                }

                //packingList?.ItemStyles?.FirstOrDefault()?.OrderDetails
                //    .OrderBy(x => x.SizeSortIndex).ToList().ForEach(o =>
                //   {
                //       if (!_dicPositionColumnPackingLine.ContainsKey(o.Size))
                //           _dicPositionColumnPackingLine.Add(o.Size, col);
        ...
        private static void FillDataSTDPL(ExcelWorksheet workSheet, Invoice invoice)
        {
                              ...
                              x?.ItemStyles?.FirstOrDefault()?.ColorCode))
            {
                ...
                var col = _colSize;
                var sizeList = GetSizeList(packinglist);
                foreach (var size in sizeList.OrderBy(x => x.Key))
                {
                    workSheet.Cells[_maxRow, col].Value = size.Value;
                    col++;
                }
                    ...
                    foreach (var packingLine in sortPackingLines)
                    {
                        ...
                        if (firstRow || packingLine.LSStyle == lsStyle)
                        {
                            if (firstRow)
                            {
                                ...
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[packID + packingLine.Size]].Value = packingLine.QuantitySize * packingLine.TotalCarton;
                                if (packingLine.BoxDimensionCode.Contains("*"))
                                {
                                    workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[LENGTH]].Value = packingLine.BoxDimensionCode.Split("*")[0].Trim();
                                    workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[WIDTH]].Value = packingLine.BoxDimensionCode.Split("*")[1].Trim();
                ...
                else
                {
                    ...
                    foreach (var packingLine in sortPackingLines)
                    {
                        ...
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNIT]].Value = unit;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[packID + packingLine.Size]].Value = packingLine.Quantity * packingLine.TotalCarton * styleCount;
                        if (packingLine.BoxDimensionCode.Contains("*"))
                        {
                            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[LENGTH]].Value = packingLine.BoxDimensionCode.Split("*")[0].Trim();
                            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[WIDTH]].Value = packingLine.BoxDimensionCode.Split("*")[1].Trim();
            ...
            _maxRow++;
            workSheet.Cells[_maxRow, _colEndSize - 1].Value = addr1;    // invoice.Company.DisplayAddress.Substring(0, 57)?.ToUpper();
            workSheet.Cells[_maxRow, _colEndSize - 1].Style.Font.SetFromFont(new Font("Times New Roman", 11));

            _maxRow++;
            workSheet.Cells[_maxRow, _colEndSize - 1].Value = addr2;    // invoice.Company.DisplayAddress.Substring(59)?.ToUpper();
            workSheet.Cells[_maxRow, _colEndSize - 1].Style.Font.SetFromFont(new Font("Times New Roman", 11));
        }
        ...
        private static void GetMaxQuantitySize(Invoice invoice)
        {
            _dicSizeList = new Dictionary<int, string>();
            foreach(var data in invoice?.PackingList)
            {
                var listSize = GetSizeList(data);

                if (_dicSizeList.Count() < listSize.Count())
                    _dicSizeList = listSize;
            }
            ...
            //foreach(var data in invoice?.PackingList)
            //{
            //    if ((data?.ItemStyles?.FirstOrDefault().OrderDetails.Count() ?? 0) > _countSize)
            //        _countSize = data?.ItemStyles?.FirstOrDefault().OrderDetails.Count() ?? 0;
            //}
        }

        public static Dictionary<int, string> GetSizeList(PackingList packingList)
        {
            var listSize = new Dictionary<int, string>();
            var i = 1;
            ...
            .OrderBy(x => x.LSStyle))
            {
                ...
                {
                    if (!listSize.ContainsValue(o.Size))
                    {
                        listSize.Add(i++, o.Size);
                    }
                });
            }
        ...
        private static void SplitAddress(List<string> addressList, ref string addr1, ref string addr2)
        {
