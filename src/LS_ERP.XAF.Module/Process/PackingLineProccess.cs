...
{
    public class PackingLineProccess
    {
        public static List<PackingLine> Generate(PackingType type, BoxDimension dimension, BoxDimension innerDimension,
            ...
            List<PackingOverQuantity> overQuantities, bool IsRemain,
            out List<PackingOverQuantity> outOverQuantites, ref int lastNoCartonShortShip)
        {
            var packingLines = new List<PackingLine>();
            var totalQuantitySize = 0;
            var noCartonShortShip = 0;
            int SequenceNo = 0;
                    ...
                    if (!IsRemain)
                    {
                        if (totalQuantity > 0)
                        {
                                ...
                                .OrderBy(x => x.SizeSortIndex))
                            {
                                ...
                                if (noCartonShortShip > 0)
                                {
                                    CartonNo += noCartonShortShip;
                                }
                                ...
                                if (totalCarton > 0)
                                {
                                    ...
                                    noCartonShortShip = orderDetailForPacking?.NoCartonShortShip ?? 0;
                        ...
                        lastNoCartonShortShip = noCartonShortShip;
                    }
                    else
                    {
                        bool firstSize = true;
                        if (ratios.Find(x => x.Ratio > 0) != null)
                        {
                                ...
                                .OrderBy(x => x.SizeSortIndex))
                            {
                                ...
                                {
                                    if (lastNoCartonShortShip > 0 && firstSize)
                                    {
                                        firstSize = false;
                                        CartonNo += lastNoCartonShortShip;
                                    }
                ...
                case PackingType.AssortedSizeSolidColor:
                    if (!IsRemain)
                    {
                        ...
                        foreach (var sizePacking in orderDetailForPackings)
                        {
                            ...
                            if (sizePacking.NoCartonShortShip > 0)
                            {
                                lastNoCartonShortShip = sizePacking.NoCartonShortShip ?? 0;
                            }
                        }
                    ...
                    else
                    {
                        ...
                        foreach (var overQuantity in overQuantities
                            .OrderBy(x => x.SizeSortIndex))
                        {
                            if (lastNoCartonShortShip > 0)
                            {
                                localCartonNo = CartonNo + lastNoCartonShortShip;
                            }
                            else if (lastNoCartonShortShip == 0)
                            {
                                orderDetailForPackings.ForEach(x =>
                                {
                                    if (x.NoCartonShortShip > 0)
                                    {
                                        noCartonShortShip = x.NoCartonShortShip ?? 0;
                                    }

                                });
                                localCartonNo = CartonNo + noCartonShortShip;
                            }
