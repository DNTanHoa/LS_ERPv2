...
{
    public class ExportPackingList_Win_ViewController : ExportPackingList_ViewController
    {
        ...
        private const string COLOR_SIZE = "color/size";
        private const string COLOR_CODE = "COLOR Code";
        private const string SIZE_HM = "Size";
        private const string UNITS_BUNDLE = "Units/ bundle";
        private const string BUNDLE_CARTON = "bundle/ carton";
        ...
        private decimal _totalColorUnits = 0;
        private int _rowSum = 0;
        private Dictionary<int, string> _dicSizeList;

        public override void ExportPackingList_Execute(object sender, SimpleActionExecuteEventArgs e)
        {
            base.ExportPackingList_Execute(sender, e);
            MessageOptions options = null;
            var export = View.CurrentObject as PackingList;
            var errorMessage = "";
            ...
            _itemStyle = export.ItemStyles.FirstOrDefault();
            ...
            foreach(var style in export?.ItemStyles)
            {
            ...
            CheckAssortedSize_AssortedColor(export);

            _countSize = _itemStyle.OrderDetails.Count();

            SaveFileDialog dialog = new SaveFileDialog();

            string fileName = export.CustomerID == "PU" ?
                              _itemStyle.LSStyle.Replace('/', '-') : _LSStyle; //export.SalesContracts.Number.Replace('/', '-');

            //var strLSStyles = export?.LSStyles.Split(';');
            //if (strLSStyles.Count() > 1)
            //{
            //    foreach (var item in strLSStyles)
            //    {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    switch (export.CustomerID)
                    {
                        ...
                        case "HM":
                            {
                                var stream = CreateExcelFile_HM(export);
                                var buffer = stream as MemoryStream;
        ...
        #region PUMA
        private Stream CreateExcelFile(PackingList packingList, Stream stream = null)
        {
            ...
            string Title = "Sheet";
            var sortThumbnail = packingList.PackingListImageThumbnails.OrderBy(x => x.SortIndex).ToList();
            var salesContractDetail = ObjectSpace.GetObjects<SalesContractDetail>(
                    CriteriaOperator.Parse(" [LSStyle] = ? AND [FactoryDate] >= #2022-08-10 00:00:00#", _itemStyle.LSStyle));

            byte[] shippingImage1 = null;
            byte[] shippingImage2 = null;
            byte[] dontShortShipImage = null;
            byte[] oclImage = null;

            if (sortThumbnail.Count > 0)
            {
                if (sortThumbnail.Count == 2)
                {
                    shippingImage1 = sortThumbnail[0].ImageData;
                    shippingImage2 = sortThumbnail[0].ImageData;
                    dontShortShipImage = sortThumbnail[1].ImageData;
                }
                else if (sortThumbnail.Count > 2)
                {
                    shippingImage1 = sortThumbnail[0].ImageData;
                    shippingImage2 = sortThumbnail[1].ImageData;
                    dontShortShipImage = sortThumbnail[2].ImageData;

                }
            }

            if (salesContractDetail != null && salesContractDetail.Count() > 0)
            {
                PackingListImageThumbnail ocl = new PackingListImageThumbnail();
                ocl.ImageUrl = ConfigurationManager.AppSettings.Get("LogoPackingOCL").ToString();
                ocl.SortIndex = 3;
                oclImage = ocl.ImageData;
            }
            ...
            using (var excelPackage = new ExcelPackage(stream ?? new MemoryStream()))
            {
                ...
                CreateHeaderPage(workSheet, packingList);
                CreateHeaderPackingLine(workSheet, packingList);
                FillDataPackingLine(workSheet, packingList);
                SummaryQuantitySize(workSheet, packingList, dontShortShipImage, oclImage);
                TotalCarton(workSheet);
                ShippingMark(workSheet, packingList, shippingImage1, shippingImage2);
                ...
                //workSheet.Column(5).Width = 0.3;

                workSheet.Column(4).AutoFit(13, 35);
                workSheet.Column(4).Style.WrapText = true;

                for (int i = 5; i < _countSize + 5; i++)
                {
                    workSheet.Column(i).AutoFit(3, 8);
                }
        ...
        public void CreateHeaderPage(ExcelWorksheet workSheet, PackingList packingList)
        {
            workSheet.Cells[1, 1].Value = "PACKING LIST";
            workSheet.Cells[1, 1].Style.Font.Bold = true;
            workSheet.Cells[1, 1].Style.Font.Color.SetColor(Color.Blue);

            int posCustomer = 9 + _countSize;
            ...
            workSheet.Cells[_maxRow, 4].Value = _LSStyle;
            workSheet.Cells[_maxRow, posCustomer].Value = "GBP PO NO:";
            workSheet.Cells[_maxRow, posCustomer, 2, posCustomer + 2].Merge = true;
            workSheet.Cells[_maxRow, posCustomer].Style.Font.Bold = true;
            workSheet.Cells[_maxRow, posCustomer + 3].Value = _itemStyle.PurchaseOrderNumber;
            workSheet.Cells[_maxRow, posCustomer + 3, 2, posCustomer + 6].Merge = true;
            ...
            workSheet.Cells[_maxRow, posCustomer].Value = "CUST CO NO: ";
            workSheet.Cells[_maxRow, posCustomer].Style.Font.Bold = true;
            workSheet.Cells[_maxRow, posCustomer, _maxRow, posCustomer + 2].Merge = true;
            workSheet.Cells[_maxRow, posCustomer + 3].Value = _itemStyle.CustomerCodeNo;
            workSheet.Cells[_maxRow, posCustomer + 3, _maxRow, posCustomer + 6].Merge = true;

            _maxRow++;
            workSheet.Cells[_maxRow, 1].Value = "DISTINATION:";
            workSheet.Cells[_maxRow, 1].Style.Font.Bold = true;
            workSheet.Cells["A" + _maxRow + ":C" + _maxRow].Merge = true;
            workSheet.Cells[_maxRow, 4].Value = _itemStyle.DeliveryPlace;
            workSheet.Cells[_maxRow, posCustomer].Value = "CUST CODE:";
            workSheet.Cells[_maxRow, posCustomer].Style.Font.Bold = true;
            workSheet.Cells[_maxRow, posCustomer, _maxRow, posCustomer + 2].Merge = true;
            workSheet.Cells[_maxRow, posCustomer + 3].Value = _itemStyle.CustomerCode;
            workSheet.Cells[_maxRow, posCustomer + 3, _maxRow, posCustomer + 6].Merge = true;

            _maxRow++;
            workSheet.Cells[_maxRow, 1].Value = "Delivery group:";
            workSheet.Cells[_maxRow, 1].Style.Font.Bold = true;
            workSheet.Cells["A" + _maxRow + ":C" + _maxRow].Merge = true;
            workSheet.Cells[_maxRow, 4].Value = packingList.DeliveryGroup;
            workSheet.Cells[_maxRow, posCustomer].Value = "UCUST CODE:";
            workSheet.Cells[_maxRow, posCustomer].Style.Font.Bold = true;
            workSheet.Cells[_maxRow, posCustomer, _maxRow, posCustomer + 2].Merge = true;
            workSheet.Cells[_maxRow, posCustomer + 3].Value = _itemStyle.UCustomerCode;
            workSheet.Cells[_maxRow, posCustomer + 3, _maxRow, posCustomer + 6].Merge = true;
            ...
            workSheet.Cells[_maxRow, 1].Style.Font.Bold = true;
            workSheet.Cells["A" + _maxRow + ":C" + _maxRow].Merge = true;
            workSheet.Cells[_maxRow, 4].Value = packingList.ShippingMethodCode;
            workSheet.Cells[_maxRow, posCustomer].Value = "UCUST CODE NO:";
            workSheet.Cells[_maxRow, posCustomer].Style.Font.Bold = true;
            workSheet.Cells[_maxRow, posCustomer, _maxRow, posCustomer + 2].Merge = true;
            workSheet.Cells[_maxRow, posCustomer + 3].Value = _itemStyle.UCustomerCodeNo;
            workSheet.Cells[_maxRow, posCustomer + 3, _maxRow, posCustomer + 6].Merge = true;

            _maxRow++;
        }

        public void CreateHeaderPackingLine(ExcelWorksheet workSheet, PackingList packinglist)
        {
            int column = 1;
            _maxRow++;
            ...
            for (int i = 0; i < _countSize; i++)
            {
                ...
                if (!string.IsNullOrEmpty(size))
                {
                    ...
                    {
                        workSheet.Cells[_maxRow + 1, column].EntireColumn.Width = 3.7;
                    }
                    column++;
                }
            }
            ...
            if (!_isCanada)
            {
                ...
                {
                    if (_dicAlphabel.TryGetValue(column, out string prepackTotalQtyCharacter))
                    {
                        range = prepackTotalQtyCharacter + _maxRow + ":" + prepackTotalQtyCharacter + (_maxRow + 1);
                        workSheet.Cells[_maxRow, column].Value = PREPACK_TOTAL_QTY;
                        workSheet.Cells[_maxRow, column].Style.Font.Bold = true;
                        workSheet.Cells[range].Merge = true;
                        workSheet.Cells[range].Style.WrapText = true;
                        workSheet.Column(column).Width = 7;
                        _dicPositionColumnPackingLine.Add(PREPACK_TOTAL_QTY, column);
                    }

                    column++;

                    if (_dicAlphabel.TryGetValue(column, out string noOfQtyPrepackCharacter))
                    {
                        range = noOfQtyPrepackCharacter + _maxRow + ":" + noOfQtyPrepackCharacter + (_maxRow + 1);
                        workSheet.Cells[_maxRow, column].Value = NO_OF_QTY_PREPACK;
                        workSheet.Cells[_maxRow, column].Style.Font.Bold = true;
                        workSheet.Cells[range].Merge = true;
                        workSheet.Cells[range].Style.WrapText = true;
                        workSheet.Column(column).Width = 7;
                        _dicPositionColumnPackingLine.Add(NO_OF_QTY_PREPACK, column);
                    }
                    column++;
                }
                ...
                if (_isAssortedSize_AssortedColor)
                {
                    if (_dicAlphabel.TryGetValue(column, out string PrepackQuantityCharacter))
                    {
                        range = PrepackQuantityCharacter + _maxRow + ":" + PrepackQuantityCharacter + (_maxRow + 1);
                        workSheet.Cells[_maxRow, column].Value = PREPACK_QTY;
                        workSheet.Cells[_maxRow, column].Style.Font.Bold = true;
                        workSheet.Cells[range].Merge = true;
                        workSheet.Cells[range].Style.WrapText = true;
                        workSheet.Column(column).Width = 7.5;
                        _dicPositionColumnPackingLine.Add(PREPACK_QTY, column);
                    }
            ...
            if (_isCanada)
            {   //////////// FOR CANADA
                if (_dicAlphabel.TryGetValue(column, out string noInnerOfCharacter))
                {
                    range = noInnerOfCharacter + _maxRow + ":" + noInnerOfCharacter + (_maxRow + 1);
                    workSheet.Cells[_maxRow, column].Value = NO_OF_INNER_CARTONS;
                    workSheet.Cells[_maxRow, column].Style.Font.Bold = true;
                    workSheet.Cells[_maxRow, column].Style.WrapText = true;
                    workSheet.Cells[range].Merge = true;
                    workSheet.Column(column).Width = 7;
                    _dicPositionColumnPackingLine.Add(NO_OF_INNER_CARTONS, column);
                }

                column++;

                if (_dicAlphabel.TryGetValue(column, out string innerCartonDimensionCharacter))
                {
                    workSheet.Cells[_maxRow, column].Value = INNER_CARTON_DIMENSION;
                    workSheet.Cells[_maxRow, column].Style.Font.Bold = true;
                    workSheet.Cells[_maxRow, column].Style.WrapText = true;

                    _dicPositionColumnPackingLine.Add(INNER_LENGTH, column);
                    workSheet.Column(column++).Width = 3.7;
                    _dicPositionColumnPackingLine.Add(INNER_WIDTH, column);
                    workSheet.Column(column++).Width = 3.7;
                    _dicPositionColumnPackingLine.Add(INNER_HEIGHT, column);
                    workSheet.Column(column).Width = 3.7;

                    if (_dicAlphabel.TryGetValue(column, out string endInnerCartonDimensionCharacter))
                    {
                        range = innerCartonDimensionCharacter + _maxRow + ":" + endInnerCartonDimensionCharacter + (_maxRow + 1);
                        workSheet.Cells[range].Merge = true;
                    }
                }
                ...
                if (_dicAlphabel.TryGetValue(column, out string unitsMasterOfCharacter))
                {
                    range = unitsMasterOfCharacter + _maxRow + ":" + unitsMasterOfCharacter + (_maxRow + 1);
                    workSheet.Cells[_maxRow, column].Value = UNITS_MASTER_CARTON;
                    workSheet.Cells[_maxRow, column].Style.Font.Bold = true;
                    workSheet.Cells[_maxRow, column].Style.WrapText = true;
                    workSheet.Cells[range].Merge = true;
                    workSheet.Column(column).Width = 7;
                    _dicPositionColumnPackingLine.Add(UNITS_MASTER_CARTON, column);
                }
            ...
            if (_dicAlphabel.TryGetValue(column, out string noOfCharacter))
            {
                ...
                if (_isCanada)
                {
                    workSheet.Cells[_maxRow, column].Value = NO_OF_MASTER_CARTONS;
                    _dicPositionColumnPackingLine.Add(NO_OF_MASTER_CARTONS, column);
                }
                else
                {
            ...
            if (_dicAlphabel.TryGetValue(column, out string cartonDimensionCharacter))
            {
                if (_isCanada)
                {
                    workSheet.Cells[_maxRow, column].Value = MASTER_CARTON_DIMENSION;
                }
                else
                {
        ...
        public void FillDataPackingLine(ExcelWorksheet workSheet, PackingList packinglist)
        {
            ...
            decimal measM3 = 0;
            int preFromNo = (int)sortPackingLines[0].FromNo;
            int preToNo = (int)sortPackingLines[0].ToNo;
            bool firstRow = true;
            bool isAssortedSize = false;
            bool isSolidSize = true;

            // FILL DATA
            foreach (var packingLine in sortPackingLines)
            {
                if (preFromNo == packingLine.FromNo && preToNo == packingLine.ToNo && firstRow)
                {
                    firstRow = false;
                    measM3 = (decimal)(packingLine.Length / 100 * packingLine.Width / 100 *
                                           packingLine.Height / 100 * (decimal)packingLine.TotalCarton);

                    switch (packingLine.PrePack)
                    {
                        case "A":
                        case "Assorted Size - Solid Color":
                            {
                                // add total carton / master carton
                                int totalCarton = (int)packingLine.TotalCarton;
                                if (_dicTotalCartons.TryGetValue(packingLine.BoxDimensionCode, out int cartonQty))
                                {
                                    if (!_isAssortedSize_AssortedColor)
                                    {
                                        cartonQty += totalCarton;
                                    }
                                ...
                                {
                                    _dicTotalCartons[packingLine.BoxDimensionCode] = totalCarton;
                                }

                                if (_isCanada)
                                {
                                    // add total inner carton
                                    if (!string.IsNullOrEmpty(packingLine.InnerBoxDimensionCode))
                                    {
                                        if (_dicTotalInnerCartons.TryGetValue(packingLine.InnerBoxDimensionCode, out int innerCartonQty))
                                        {
                                            innerCartonQty += (int)packingLine.TotalCarton * (int)packingLine.PackagesPerBox;

                                            _dicTotalInnerCartons[packingLine.InnerBoxDimensionCode] = innerCartonQty;
                                        }
                                        else
                                        {
                                            _dicTotalInnerCartons[packingLine.InnerBoxDimensionCode] = (int)packingLine.TotalCarton *
                                                                                                       (int)packingLine.PackagesPerBox;
                                        }
                                    }
                                }
                                ...
                                if (_isAssortedSize_AssortedColor)
                                {
                                    ...
                                    _unitCarton += (decimal)packingLine.QuantityPerCarton;
                                    ...
                                    _totalNetWeight += Math.Round((decimal)netWeight, 3);
                                    _totalGrossWeight += Math.Round((decimal)grossWeight, 3);
                                }
                                else
                                {
                                    _totalNoOfCarton += (decimal)packingLine.TotalCarton;
                ...
                if (preFromNo != packingLine.FromNo && preToNo != packingLine.ToNo)
                {
                    _maxRow++;
                    _mergeColumn++;
                    preFromNo = (int)packingLine.FromNo;
                    preToNo = (int)packingLine.ToNo;

                    if (isSolidSize && !_isAssortedSize_AssortedColor)
                    {
                        _unitCarton = 0;
                        netWeight = 0;
                        grossWeight = 0;
                ...
                if (_isAssortedSize_AssortedColor)
                {
                    workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[COLOR]].Value = packingLine.Color + " "
                            + packingLine.PackingList.ItemStyles.First(x => x.ColorCode == packingLine.Color).ColorName;

                    _dicShipmentQuantity[packingLine.Size + packingLine.Color] = (decimal)packingLine.Quantity;
                }
                else
                {
                    workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[COLOR]]
                             .Value = packingLine.Color + " " + _itemStyle.ColorName;
                }
                        ...
                        case "Assorted Size - Solid Color":
                            {
                                ...
                                if (_isAssortedSize_AssortedColor)
                                {
                                    workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[PREPACK_QTY]]
                                        .Value = packingLine.QuantityPerPackage;
                                }
                                else
                                {
                                    workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[PREPACK_TOTAL_QTY]]
                                         .Value = packingLine.QuantityPerPackage;
                                    workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[NO_OF_QTY_PREPACK]]
                                             .Value = packingLine.PackagesPerBox;
                                }


                                decimal totalQuantitySize = (decimal)packingLine.QuantitySize * (decimal)packingLine.TotalCarton;

                                // add total quantity
                                if (_dicTotalQuantitySize.TryGetValue(packingLine.Size, out decimal quantitySize))
                                {
                                    quantitySize += totalQuantitySize;

                                    _dicTotalQuantitySize[packingLine.Size] = quantitySize;
                                }
                                else
                                {
                                    _dicTotalQuantitySize[packingLine.Size] = totalQuantitySize;
                                }

                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON]]
                                        .Value = packingLine.QuantityPerCarton;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[NO_OF_CARTON]]
                                         .Value = packingLine.TotalCarton;

                                // GET DATA TOTAL FOR ASSORTED SIZE
                                if (isAssortedSize)
                                {
                                    int totalCarton = (int)packingLine.TotalCarton;
                                    // add total carton
                                    ...
                                    isAssortedSize = false;
                                }
                            }
                            break;
                        default:
                            {
                                decimal totalQuantity = (decimal)((decimal)packingLine.TotalCarton * packingLine.Quantity);

                                // add total quantity
                                if (_dicTotalQuantitySize.TryGetValue(packingLine.Size, out decimal quantity))
                                {
                                    quantity += totalQuantity;
                                    _dicTotalQuantitySize[packingLine.Size] = quantity;
                                }
                                else
                                {
                                ...
                                // GET DATA TOTAL
                                if (!firstRow && !isAssortedSize && isSolidSize)
                                {
                                    //workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON]]
                                    ...
                                    if (_dicTotalCartons.TryGetValue(packingLine.BoxDimensionCode, out int cartonQty))
                                    {
                                        if (!_isAssortedSize_AssortedColor && _unitCarton == 0)
                                        {
                                    ...
                                    if (_isAssortedSize_AssortedColor)
                                    {
                                        ...
                                        grossWeight += (decimal)packingLine.GrossWeight;
                                        _totalGrossWeight += Math.Round((decimal)packingLine.GrossWeight, 3);
                                    }
                                    else
                                    {

                                        if (packingLine.FromNo == packingLine.ToNo)
                                        {
                                            if (_unitCarton == 0)
                                            {
                                                _totalNoOfCarton += (decimal)packingLine.TotalCarton;
                                           ...
                                           packingLine.Height / 100 * (decimal)packingLine.TotalCarton);
                                                totalMeasM3 += Math.Round(measM3, 3);
                                                grossWeight += (decimal)packingLine.BoxDimension.Weight;
                                                _totalGrossWeight += Math.Round((decimal)packingLine.BoxDimension.Weight, 3);
                                            }

                                            grossWeight += (decimal)packingLine.NetWeight;
                                            _totalGrossWeight += Math.Round((decimal)packingLine.NetWeight, 3);
                                            _unitCarton += (decimal)packingLine.Quantity;
                                            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON]]
                                            .Value = _unitCarton;
                                        }
                                        else
                                        {
                                            _totalNoOfCarton += (decimal)packingLine.TotalCarton;
                                            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON]]
                                            .Value = (decimal)packingLine.Quantity;
                                            measM3 = (decimal)(packingLine.Length / 100 * packingLine.Width / 100 *
                                           packingLine.Height / 100 * (decimal)packingLine.TotalCarton);
                                            totalMeasM3 += Math.Round(measM3, 3);
                                    ...
                                    _totalUnits += (decimal)packingLine.TotalQuantity;
                                    _totalNetWeight += Math.Round((decimal)packingLine.NetWeight, 3);

                                }
                                else if (!firstRow && isAssortedSize)
                                {
                                    workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON]]
                                         .Value = packingLine.QuantityPerCarton;
                        ...
                        case "Assorted Size - Solid Color":
                            {
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON]]
                                    .Value = packingLine.QuantityPerPackage;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[NO_OF_INNER_CARTONS]]
                                    .Value = packingLine.PackagesPerBox;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[INNER_LENGTH]]
                                    .Value = packingLine.InnerLength;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[INNER_WIDTH]]
                                    .Value = packingLine.InnerWidth;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[INNER_HEIGHT]]
                                    .Value = packingLine.InnerHeight;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_MASTER_CARTON]]
                                    .Value = packingLine.QuantityPerCarton;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[NO_OF_MASTER_CARTONS]]
                                    .Value = packingLine.TotalCarton;

                                decimal totalQuantity = (decimal)((decimal)packingLine.TotalCarton * packingLine.Quantity
                                                                           * packingLine.PackagesPerBox);
                ...
                if (!_isAssortedSize_AssortedColor)
                {
                    ...
                    {
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_UNITS]].Value = _unitCarton;
                    }
                }
                else
                {
            ...
            {
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[NO_OF_MASTER_CARTONS]].Value = _totalNoOfCarton;
            }
            ...
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[MEAS_M3]].Value = Math.Round(totalMeasM3, 3);
            ...
            if (_dicAlphabel.TryGetValue(_maxColumn, out string maxColumnCharacter))
            {
                ...
                if (!_isCanada)
                {
                    range = _dicAlphabel[_dicPositionColumnPackingLine[NO_OF_CARTON]] + _maxRow + ":" +
                            _dicAlphabel[_dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT]] + _maxRow;
                }
                else
                {
                    range = _dicAlphabel[_dicPositionColumnPackingLine[NO_OF_MASTER_CARTONS]] + _maxRow + ":" +
                            _dicAlphabel[_dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT]] + _maxRow;
                }
            ...
            {
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[NO_OF_MASTER_CARTONS]].Value = "Cnts";
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[NO_OF_MASTER_CARTONS]].Style.Font.Bold = true;
            }
            ...
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[NET_WEIGHT]].Style.Font.Bold = true;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[GROSS_WEIGHT]].Value = "Kg";
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[GROSS_WEIGHT]].Style.Font.Bold = true;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT]].Value = "Kg";
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT]].Style.Font.Bold = true;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT]].Value = "Kg";
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT]].Style.Font.Bold = true;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON],
                            _maxRow, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT]]
            ...
            if (_isAssortedSize_AssortedColor)
            {
                MergeRow(workSheet, packinglist);
            }
        }

        private void MergeRow(ExcelWorksheet workSheet, PackingList packingList)
        {
            workSheet.Cells[10, _dicPositionColumnPackingLine[FROM_NO],
                               10 + _mergeColumn, _dicPositionColumnPackingLine[FROM_NO]].Merge = true;
            workSheet.Cells[10, _dicPositionColumnPackingLine[FROM_NO] + 1,
                               10 + _mergeColumn, _dicPositionColumnPackingLine[FROM_NO] + 1].Merge = true;
            workSheet.Cells[10, _dicPositionColumnPackingLine[TO_NO],
                               10 + _mergeColumn, _dicPositionColumnPackingLine[TO_NO]].Merge = true;

            workSheet.Cells[10, _dicPositionColumnPackingLine[UNITS_CARTON],
                               10 + _mergeColumn, _dicPositionColumnPackingLine[UNITS_CARTON]].Merge = true;
            workSheet.Cells[10, _dicPositionColumnPackingLine[UNITS_CARTON],
                               10 + _mergeColumn, _dicPositionColumnPackingLine[UNITS_CARTON]].Value = _unitCarton;
            workSheet.Cells[10, _dicPositionColumnPackingLine[NO_OF_CARTON],
                               10 + _mergeColumn, _dicPositionColumnPackingLine[NO_OF_CARTON]].Merge = true;

            var netWeight = _totalNetWeight / _totalNoOfCarton;
            workSheet.Cells[10, _dicPositionColumnPackingLine[NET_WEIGHT],
                               10 + _mergeColumn, _dicPositionColumnPackingLine[NET_WEIGHT]].Merge = true;
            workSheet.Cells[10, _dicPositionColumnPackingLine[NET_WEIGHT],
                               10 + _mergeColumn, _dicPositionColumnPackingLine[NET_WEIGHT]]
                               .Value = netWeight;

            var grossWeight = netWeight + (decimal)packingList?.PackingLines.First().BoxDimension?.Weight;
            workSheet.Cells[10, _dicPositionColumnPackingLine[GROSS_WEIGHT],
                               10 + _mergeColumn, _dicPositionColumnPackingLine[GROSS_WEIGHT]].Merge = true;
            workSheet.Cells[10, _dicPositionColumnPackingLine[GROSS_WEIGHT],
                               10 + _mergeColumn, _dicPositionColumnPackingLine[GROSS_WEIGHT]].Value = grossWeight;

            workSheet.Cells[10, _dicPositionColumnPackingLine[LENGTH],
                               10 + _mergeColumn, _dicPositionColumnPackingLine[LENGTH]].Merge = true;

            workSheet.Cells[10, _dicPositionColumnPackingLine[WIDTH],
                              10 + _mergeColumn, _dicPositionColumnPackingLine[WIDTH]].Merge = true;
            workSheet.Cells[10, _dicPositionColumnPackingLine[HEIGHT],
                              10 + _mergeColumn, _dicPositionColumnPackingLine[HEIGHT]].Merge = true;
            workSheet.Cells[10, _dicPositionColumnPackingLine[MEAS_M3],
                              10 + _mergeColumn, _dicPositionColumnPackingLine[MEAS_M3]].Merge = true;

            workSheet.Cells[10, _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT],
                              10 + _mergeColumn, _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT]].Merge = true;
            workSheet.Cells[10, _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT],
                              10 + _mergeColumn, _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT]].Value = _totalNetWeight;

            _totalGrossWeight = grossWeight * _totalNoOfCarton;
            workSheet.Cells[10, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT],
                              10 + _mergeColumn, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT]].Value = _totalGrossWeight;
            workSheet.Cells[10, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT],
                              10 + _mergeColumn, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT]].Merge = true;

            workSheet.Cells[_maxRow - 3, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT]].Value = _totalGrossWeight;
        }

        public void SummaryQuantitySize(ExcelWorksheet workSheet, PackingList packingList, byte[] dontShortShip, byte[] oclImage)
        {
            string range = "";
            int columnSize = _dicPositionColumnPackingLine[SIZE];
            int columnTotalUnit = _dicPositionColumnPackingLine[UNITS_CARTON];
            var dicShortOverQty = new Dictionary<string, decimal>();
            var dicShortOverPercent = new Dictionary<string, decimal>();
            ...
            {
                // dont short ship
                if (packingList.DontShortShip != null && packingList.DontShortShip == true)
                {
                    Image logo1 = LogoDownload(dontShortShip);
                    var pictureRight = workSheet.Drawings.AddPicture("pictureRightDontShortShip", logo1);
                    pictureRight.SetPosition(_maxRow, 0, _dicPositionColumnPackingLine[NET_WEIGHT], 0);
                }

                _maxRow++;
                workSheet.Cells[_maxRow, columnSize].Value = SIZE;
                workSheet.Cells[_maxRow, columnSize].Style.Font.Bold = true;
                workSheet.Cells[_maxRow, columnSize, _maxRow, columnSize + _countSize - 1].Merge = true;

                workSheet.Cells[_maxRow, columnTotalUnit].Value = TOTAL_UNITS;
                workSheet.Cells[_maxRow, columnTotalUnit].Style.Font.Bold = true;
                workSheet.Cells[_maxRow, columnTotalUnit, _maxRow + 1, columnTotalUnit].Merge = true;
                workSheet.Cells[_maxRow, columnTotalUnit, _maxRow + 1, columnTotalUnit].Style.WrapText = true;
                workSheet.Column(columnTotalUnit).Width = 7;

                // OCL image
                if (oclImage != null)
                {
                    Image logoOCL = LogoDownload(oclImage);
                    var pictureRightBottom = workSheet.Drawings.AddPicture("pictureRightBottomOCL", logoOCL);
                    pictureRightBottom.SetPosition(_maxRow + 3, 0, _dicPositionColumnPackingLine[NET_WEIGHT], 0);
                }
                ...
                workSheet.Cells[_maxRow + 1, 4].Value = "Order Qty";
                workSheet.Cells[_maxRow + 2, 4].Value = "Shipment Qty";
                workSheet.Cells[_maxRow + 3, 4].Value = "Short/ Over Qty";
                workSheet.Cells[_maxRow + 4, 4].Value = "Short/ Over %";
                decimal totalOrderQty = 0;
                decimal totalShorOverQty = 0;


                foreach (var orderDetail in _itemStyle.OrderDetails)
                {
                    if (_dicPositionColumnPackingLine.TryGetValue(orderDetail.Size, out int columnSizeName))
                    {
                        decimal shortOverQty = (decimal)(_dicTotalQuantitySize[orderDetail.Size] - orderDetail.Quantity);
                        decimal shortOverPercent = Math.Round((shortOverQty / (decimal)orderDetail.Quantity) * 100, 2);
                        totalOrderQty += (decimal)orderDetail.Quantity;
                        totalShorOverQty += shortOverQty;

                        workSheet.Cells[_maxRow, columnSizeName].Value = orderDetail.Size;
                        workSheet.Cells[_maxRow + 1, columnSizeName].Value = orderDetail.Quantity;
                        workSheet.Cells[_maxRow + 2, columnSizeName].Value = _dicTotalQuantitySize[orderDetail.Size];
                        workSheet.Cells[_maxRow + 3, columnSizeName].Value = shortOverQty;
                        workSheet.Cells[_maxRow + 4, columnSizeName].Value = shortOverPercent + " %";
                    }
                }

                workSheet.Cells[_maxRow + 1, columnTotalUnit].Value = totalOrderQty;
                workSheet.Cells[_maxRow + 2, columnTotalUnit].Value = _totalUnits;
                workSheet.Cells[_maxRow + 3, columnTotalUnit].Value = totalShorOverQty;
                workSheet.Cells[_maxRow + 4, columnTotalUnit].Value = Math.Round(totalShorOverQty / totalOrderQty * 100, 2) + " %";

                _maxRow += 4;
                range += _dicAlphabel[columnTotalUnit] + _maxRow;
                using (var borderData = workSheet.Cells[range])
                {
                    borderData.Style.VerticalAlignment = ExcelVerticalAlignment.Center;
                    borderData.Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
            ...
            else
            {
                ...
                workSheet.Cells[_maxRow, columnTotalUnit, _maxRow + 1, columnTotalUnit].Style.WrapText = true;
                workSheet.Column(columnTotalUnit).Width = 7;
                workSheet.Cells[_maxRow + 1, 4].Value = "Color";
                workSheet.Cells[_maxRow + 1, 1, _maxRow + 1, 3].Merge = true;
                range = "A" + _maxRow + ":";
                ...
                if (oclImage != null)
                {
                    Image logoOCL = LogoDownload(oclImage);
                    var pictureRightBottom = workSheet.Drawings.AddPicture("pictureRightBottomOCL", logoOCL);
                    pictureRightBottom.SetPosition(_maxRow + 3 + (_mergeColumn * 2) + 2, 0, _dicPositionColumnPackingLine[NET_WEIGHT], 0);
                }

                _maxRow++;

                workSheet.Cells[_maxRow + 1, 1].Value = "Order Qty";
                workSheet.Cells[_maxRow + 1, 1, _maxRow + 1, 3].Merge = true;

                workSheet.Cells[_maxRow + 1 + (_mergeColumn * 1) + 2, 1].Value = "Shipment Qty";
                workSheet.Cells[_maxRow + 1 + (_mergeColumn * 1) + 2, 1, _maxRow + 1 + (_mergeColumn * 1) + 2, 3].Merge = true;

                workSheet.Cells[_maxRow + 3 + (_mergeColumn * 2) + 2, 1].Value = "Short/ Over Qty";
                workSheet.Cells[_maxRow + 3 + (_mergeColumn * 2) + 2, 1, _maxRow + 3 + (_mergeColumn * 2) + 2, 3].Merge = true;

                workSheet.Cells[_maxRow + 4 + (_mergeColumn * 2) + 2, 1].Value = "Short/ Over %";
                workSheet.Cells[_maxRow + 4 + (_mergeColumn * 2) + 2, 1, _maxRow + 4 + (_mergeColumn * 2) + 2, 3].Merge = true;

                for (int i = 0; i < packingList.ItemStyles.Count; i++)
                {
                    var itemStyle = packingList.ItemStyles[i];
                    decimal totalOrderQty = 0;
                    decimal totalShipmentQty = 0;
                    workSheet.Cells[_maxRow + 1, 4].Value = itemStyle.ColorCode + " " + itemStyle.ColorName;
                    workSheet.Cells[_maxRow + 1 + _mergeColumn + 2, 4].Value = itemStyle.ColorCode + " " + itemStyle.ColorName;


                    for (int j = 0; j < itemStyle.OrderDetails.Count; j++)
                    {
                        var orderDetail = itemStyle.OrderDetails[j];

                        if (_dicPositionColumnPackingLine.TryGetValue(orderDetail.Size, out int columnSizeName))
                        {
                            decimal shipmentQty = (decimal)_dicShipmentQuantity[orderDetail.Size + itemStyle.ColorCode] * _totalNoOfCarton;
                            decimal shortOverQty = (decimal)(shipmentQty - orderDetail.Quantity);
                            decimal shortOverPercent = Math.Round((shortOverQty / (decimal)orderDetail.Quantity) * 100, 2);
                            totalOrderQty += (decimal)orderDetail.Quantity;
                            totalShipmentQty += shipmentQty;

                            if (dicShortOverQty.TryGetValue(orderDetail.Size, out decimal rsShortOverQty))
                            {
                                shortOverQty += rsShortOverQty;

                            }

                            if (dicShortOverPercent.TryGetValue(orderDetail.Size, out decimal rsShortOverPercent))
                            {
                                shortOverPercent += rsShortOverPercent;

                            }

                            dicShortOverQty[orderDetail.Size] = shortOverQty;
                            dicShortOverPercent[orderDetail.Size] = shortOverPercent;
                            ...
                            if (i == 0)
                            {
                                ...
                                workSheet.Cells[_maxRow + 1, columnSizeName].Value = orderDetail.Quantity;
                                workSheet.Cells[_maxRow + 1, columnTotalUnit].Value = totalOrderQty;
                                workSheet.Cells[_maxRow + 1 + _mergeColumn + 2, columnSizeName].Value = shipmentQty;
                            }
                            else
                            {
                                workSheet.Cells[_maxRow + 1, columnSizeName].Value = orderDetail.Quantity;
                    ...
                    workSheet.Cells[_maxRow + 1 + _mergeColumn + 2, columnTotalUnit].Value = totalShipmentQty;

                    workSheet.Cells[_maxRow + 1, 1, _maxRow + 1, 3].Merge = true;
                    workSheet.Cells[_maxRow + 1 + _mergeColumn + 2, 1, _maxRow + 1 + _mergeColumn + 2, 3].Merge = true;
                    _maxRow++;
                }

                decimal totalShortOverQty = 0;
                foreach (var itemShortOverQty in dicShortOverQty)
                {
                    if (_dicPositionColumnPackingLine.TryGetValue(itemShortOverQty.Key, out int columnSizeName))
                    {
                        workSheet.Cells[_maxRow + (_mergeColumn * 2) + 2, columnSizeName].Value = itemShortOverQty.Value;
                        totalShortOverQty += (decimal)itemShortOverQty.Value;
                    }
                }
                workSheet.Cells[_maxRow + (_mergeColumn * 2) + 2, columnTotalUnit].Value = totalShortOverQty;

                decimal totalShortOverPercent = 0;
                foreach (var itemShortOverPercent in dicShortOverPercent)
                {
                    if (_dicPositionColumnPackingLine.TryGetValue(itemShortOverPercent.Key, out int columnSizeNamePercent))
                    {
                        workSheet.Cells[_maxRow + (_mergeColumn * 2) + 3, columnSizeNamePercent].Value = itemShortOverPercent.Value + " %";
                        totalShortOverPercent += (decimal)itemShortOverPercent.Value;
                    }
                }
                workSheet.Cells[_maxRow + (_mergeColumn * 2) + 3, columnTotalUnit].Value = (totalShortOverPercent * 100) + " %";

                workSheet.Cells[_maxRow + 1 + _mergeColumn + 2, 1, _maxRow + 1 + _mergeColumn + 2, 3].Merge = true;
                workSheet.Cells[_maxRow + 1, 1, _maxRow + 1, 4].Merge = true;
                _maxRow++;
        ...
        public void TotalCarton(ExcelWorksheet workSheet)
        {

            int columnLength = _dicPositionColumnPackingLine[GROSS_WEIGHT];
            int columnMeasM3 = _dicPositionColumnPackingLine[MEAS_M3];

            _maxRow++;
            workSheet.Cells[_maxRow, columnLength].Value = "TOTAL NW";
            workSheet.Cells[_maxRow, columnLength, _maxRow, columnLength + 3].Merge = true;

            workSheet.Cells[_maxRow, columnMeasM3].Value = _totalNetWeight;
            workSheet.Cells[_maxRow, columnMeasM3, _maxRow, columnMeasM3 + 1].Merge = true;
            _maxRow++;
            workSheet.Cells[_maxRow, columnLength].Value = "TOTAL GW";
            workSheet.Cells[_maxRow, columnLength, _maxRow, columnLength + 3].Merge = true;

            workSheet.Cells[_maxRow, columnMeasM3].Value = _totalGrossWeight;
            workSheet.Cells[_maxRow, columnMeasM3, _maxRow, columnMeasM3 + 1].Merge = true;
            _maxRow++;
            _maxRow++;

            _dicTotalCartons = _dicTotalCartons.OrderBy(x => x.Key).ToDictionary(x => x.Key, x => x.Value);
            string range = "";
            string rangeQty = "";
            if (!_isCanada)
            {
                range = _dicAlphabel[columnLength] + _maxRow + ":" + _dicAlphabel[columnMeasM3 + 1];
                rangeQty = _dicAlphabel[columnMeasM3] + _maxRow + ":" + _dicAlphabel[columnMeasM3 + 1];

                workSheet.Cells[_maxRow, columnLength].Value = "CTNS SUMMARY";
                workSheet.Cells[_maxRow, columnLength, _maxRow, columnLength + 3].Merge = true;
                workSheet.Cells[_maxRow, columnMeasM3].Value = "QTY";
                workSheet.Cells[_maxRow, columnMeasM3, _maxRow, columnMeasM3 + 1].Merge = true;
                workSheet.Cells[_maxRow, columnMeasM3, _maxRow, columnMeasM3 + 1]
                         .Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                _maxRow++;
                int totalCarton = 0;
                foreach (var carton in _dicTotalCartons)
                {
                    workSheet.Cells[_maxRow, columnLength].Value = carton.Key;
                    workSheet.Cells[_maxRow, columnLength, _maxRow, columnLength + 3].Merge = true;

                    workSheet.Cells[_maxRow, columnMeasM3].Value = carton.Value;
                    workSheet.Cells[_maxRow, columnMeasM3, _maxRow, columnMeasM3 + 1].Merge = true;
                    totalCarton += carton.Value;
                    _maxRow++;
                }
                workSheet.Cells[_maxRow, columnLength].Value = "Total";
                workSheet.Cells[_maxRow, columnLength].Style.Font.Bold = true;
                workSheet.Cells[_maxRow, columnLength, _maxRow, columnLength + 3].Merge = true;
                workSheet.Cells[_maxRow, columnMeasM3].Value = totalCarton;
                workSheet.Cells[_maxRow, columnMeasM3].Style.Font.Bold = true;
                workSheet.Cells[_maxRow, columnMeasM3, _maxRow, columnMeasM3 + 1].Merge = true;
                ...
                workSheet.Cells[rangeQty].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
            }
            else
            {
                /// INNER CARTON
                int columnInnerBoxDimension = _dicPositionColumnPackingLine[INNER_LENGTH];
                int columnUnitMasterCarton = _dicPositionColumnPackingLine[UNITS_MASTER_CARTON];
                range = _dicAlphabel[columnInnerBoxDimension] + _maxRow + ":"
                       + _dicAlphabel[columnUnitMasterCarton + 1];
                rangeQty = _dicAlphabel[columnUnitMasterCarton] + _maxRow + ":"
                       + _dicAlphabel[columnUnitMasterCarton + 1];

                int rowMasterBox = _maxRow;
                int rowInnerBox = _maxRow;
                workSheet.Cells[rowInnerBox, columnInnerBoxDimension].Value = "INNER CTNS";
                workSheet.Cells[rowInnerBox, columnInnerBoxDimension, rowInnerBox, columnInnerBoxDimension + 2]
                    .Merge = true;
                workSheet.Cells[rowInnerBox, columnUnitMasterCarton].Value = "QTY";
                workSheet.Cells[rowInnerBox, columnUnitMasterCarton, rowInnerBox, columnUnitMasterCarton + 1]
                    .Merge = true;
                rowInnerBox++;
                _dicTotalInnerCartons = _dicTotalInnerCartons.OrderBy(x => x.Key).ToDictionary(x => x.Key, x => x.Value);
                int totalInnerCarton = 0;
                foreach (var carton in _dicTotalInnerCartons)
                {
                    workSheet.Cells[rowInnerBox, columnInnerBoxDimension].Value = carton.Key;
                    workSheet.Cells[rowInnerBox, columnInnerBoxDimension, rowInnerBox, columnInnerBoxDimension + 2]
                        .Merge = true;

                    workSheet.Cells[rowInnerBox, columnUnitMasterCarton].Value = carton.Value;
                    workSheet.Cells[rowInnerBox, columnUnitMasterCarton, rowInnerBox, columnUnitMasterCarton + 1]
                        .Merge = true;
                    totalInnerCarton += carton.Value;
                    rowInnerBox++;
                }
                workSheet.Cells[rowInnerBox, columnInnerBoxDimension].Value = "Total";
                workSheet.Cells[rowInnerBox, columnInnerBoxDimension].Style.Font.Bold = true;
                workSheet.Cells[rowInnerBox, columnInnerBoxDimension, rowInnerBox, columnInnerBoxDimension + 2]
                    .Merge = true;
                workSheet.Cells[rowInnerBox, columnUnitMasterCarton].Value = totalInnerCarton;
                workSheet.Cells[rowInnerBox, columnUnitMasterCarton].Style.Font.Bold = true;
                workSheet.Cells[rowInnerBox, columnUnitMasterCarton, rowInnerBox, columnUnitMasterCarton + 1]
                    .Merge = true;

                range += rowInnerBox;
                rangeQty += rowInnerBox;
                // style INNER CARTON
                workSheet.Cells[range].Style.Border.Top.Style = ExcelBorderStyle.Thin;
                workSheet.Cells[range].Style.Border.Bottom.Style = ExcelBorderStyle.Thin;
                workSheet.Cells[range].Style.Border.Right.Style = ExcelBorderStyle.Thin;
                ...
                /// for MASTER CARTON
                workSheet.Cells[rowMasterBox, columnLength].Value = "MASTER CTNS";
                workSheet.Cells[rowMasterBox, columnLength, rowMasterBox, columnLength + 3].Merge = true;
                workSheet.Cells[rowMasterBox, columnMeasM3].Value = "QTY";
                workSheet.Cells[rowMasterBox, columnMeasM3, rowMasterBox, columnMeasM3 + 1].Merge = true;
                range = _dicAlphabel[columnLength] + rowMasterBox + ":" + _dicAlphabel[columnMeasM3 + 1];
                rangeQty = _dicAlphabel[columnMeasM3] + _maxRow + ":" + _dicAlphabel[columnMeasM3 + 1];
                ...
                foreach (var carton in _dicTotalCartons)
                {
                    workSheet.Cells[rowMasterBox, columnLength].Value = carton.Key;
                    workSheet.Cells[rowMasterBox, columnLength, rowMasterBox, columnLength + 3].Merge = true;

                    workSheet.Cells[rowMasterBox, columnMeasM3].Value = carton.Value;
                    workSheet.Cells[rowMasterBox, columnMeasM3, rowMasterBox, columnMeasM3 + 1].Merge = true;
                    totalCarton += carton.Value;
                    rowMasterBox++;
                }
                workSheet.Cells[rowMasterBox, columnLength].Value = "Total";
                workSheet.Cells[rowMasterBox, columnLength].Style.Font.Bold = true;
                workSheet.Cells[rowMasterBox, columnLength, rowMasterBox, columnLength + 3].Merge = true;
                workSheet.Cells[rowMasterBox, columnMeasM3].Value = totalCarton;
                workSheet.Cells[rowMasterBox, columnMeasM3].Style.Font.Bold = true;
                workSheet.Cells[rowMasterBox, columnMeasM3, rowMasterBox, columnMeasM3 + 1].Merge = true;
                range += rowMasterBox;
                rangeQty += rowMasterBox;
                // style MASTER CARTON
                workSheet.Cells[range].Style.Border.Top.Style = ExcelBorderStyle.Thin;
                workSheet.Cells[range].Style.Border.Bottom.Style = ExcelBorderStyle.Thin;
                workSheet.Cells[range].Style.Border.Right.Style = ExcelBorderStyle.Thin;
                ...
                if (rowMasterBox > rowInnerBox)
                {
                    _maxRow = rowMasterBox;
                }
                else
                {
                    _maxRow = rowInnerBox;
        ...
        public void ShippingMark(ExcelWorksheet workSheet,
            PackingList packinglist, byte[] shippingImage1, byte[] shippingImage2)
        {

            workSheet.Cells[_maxRow, 1].Value = "SHIPPING MARK:";
            workSheet.Cells[_maxRow, 1].Style.Font.Bold = true;
            _maxRow++;

            if (_isPanama)
            {
                int columnGWeight = _dicPositionColumnPackingLine[GROSS_WEIGHT];
                int columnMeasM3 = _dicPositionColumnPackingLine[MEAS_M3];

                workSheet.Cells[_maxRow, 4].Value = "Side Marks (both short sides):";
                workSheet.Cells[_maxRow, 4].Style.Font.Bold = true;
                workSheet.Cells[_maxRow, 4, _maxRow, 7].Merge = true;

                workSheet.Cells[_maxRow, 8].Value = packinglist?.RatioLeft; /// Update 2022-10-27

                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = "";
                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = PANAMA;
                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = "Customer CO. No:";
                workSheet.Cells[_maxRow, 5].Value = _itemStyle.CustomerCodeNo;
                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = "Art. No:";
                workSheet.Cells[_maxRow, 5].Value = _itemStyle.CustomerStyle + "-" +
                                                    _itemStyle.ColorCode;
                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = "Art. Name:";
                workSheet.Cells[_maxRow, 5].Value = _itemStyle.Description;

                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = "Size:";
                workSheet.Cells[_maxRow, 5, _maxRow, columnGWeight - 1].Merge = true;

                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = "Qty:";
                workSheet.Cells[_maxRow, 5, _maxRow, columnGWeight - 1].Merge = true;

                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = "Carton no:";
                workSheet.Cells[_maxRow, 5].Value = "of";

                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = "G.W.:";
                workSheet.Cells[_maxRow, 5].Value = "KG";

                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = "N.W.:";
                workSheet.Cells[_maxRow, 5].Value = "KG";

                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = "MEAS:";
                workSheet.Cells[_maxRow, 5].Value = "";

                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = "Made in Vietnam";

                Image logo = LogoDownload(shippingImage1);
                var pictureLeft = workSheet.Drawings.AddPicture("pictureLeft", logo);
                pictureLeft.SetPosition(_maxRow, 0, 0, 0);
            }
            else if (_isCanada)
            {

                /// image
                Image logo1 = LogoDownload(shippingImage1);
                var pictureLeft = workSheet.Drawings.AddPicture("pictureLeft", logo1);
                pictureLeft.SetPosition(_maxRow, 0, 0, 0);

                Image logo2 = LogoDownload(shippingImage2);
                var pictureRight = workSheet.Drawings.AddPicture("pictureRight", logo2);
                pictureRight.SetPosition(_maxRow, 0, _dicPositionColumnPackingLine[UNITS_MASTER_CARTON] - 1, 0);

                _maxRow += 6;
            }
            else if (_isJapan)
            {
                workSheet.Cells[_maxRow, 1, _maxRow + 5, 3].Merge = true;
                workSheet.Cells[_maxRow, 4].Value = "\tPJ" +
                                                    "\n\tThinkPad Tower 18F" +
                                                    "\n\t2-1-1 Osaki Shingana-ku" +
                                                    "\n\tTokyo 141 -6018 Japan" +
                                                    "\n\tCarton No:          Of" +
                                                    "\n\tMade in Vietnam";
                workSheet.Cells[_maxRow, 4, _maxRow + 5, 4 + _dicPositionColumnPackingLine[UNITS_CARTON] - 5].Merge = true;


                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON]]
                         .Value = "OCL";
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON],
                                _maxRow + 3, _dicPositionColumnPackingLine[NO_OF_CARTON]].Merge = true;
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON],
                                _maxRow + 3, _dicPositionColumnPackingLine[NO_OF_CARTON]]
                         .Style.VerticalAlignment = ExcelVerticalAlignment.Center;
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON],
                               _maxRow + 3, _dicPositionColumnPackingLine[NO_OF_CARTON]]
                        .Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

                workSheet.Cells[_maxRow + 4, _dicPositionColumnPackingLine[UNITS_CARTON],
                                _maxRow + 5, _dicPositionColumnPackingLine[NO_OF_CARTON]].Merge = true;

                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_UNITS]].Value = "G.W        KG" +
                                                                                              "\nN.W        KG" +
                                                                                              "\nMEAS";
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_UNITS],
                                _maxRow + 5, _dicPositionColumnPackingLine[NET_WEIGHT]].Merge = true;
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_UNITS],
                                _maxRow + 5, _dicPositionColumnPackingLine[NET_WEIGHT]]
                ...
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[GROSS_WEIGHT]].Value = "PJ" +
                                                                                              "\nThinkPad Tower 18F" +
                                                                                              "\n2-1-1 Osaki Shingana-ku" +
                                                                                              "\nTokyo 141 -6018 Japan" +
                                                                                              "\nCarton No:          Of" +
                ...
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[GROSS_WEIGHT],
                                _maxRow + 5, _dicPositionColumnPackingLine[MEAS_M3]].Merge = true;
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[GROSS_WEIGHT],
                                _maxRow + 5, _dicPositionColumnPackingLine[MEAS_M3]]
                         .Style.VerticalAlignment = ExcelVerticalAlignment.Center;
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[GROSS_WEIGHT],
                               _maxRow + 5, _dicPositionColumnPackingLine[MEAS_M3]]
                        .Style.HorizontalAlignment = ExcelHorizontalAlignment.Left;
                ...
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT],
                                _maxRow + 5, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT]].Merge = true;

                /// image
                Image logo1 = LogoDownload(shippingImage1);
                var pictureLeft = workSheet.Drawings.AddPicture("pictureLeft", logo1);
                pictureLeft.SetPosition(_maxRow + 2, 0, 0, 0);
                pictureLeft.SetSize(90, 45);

                Image logo2 = LogoDownload(shippingImage2);
                var pictureRight = workSheet.Drawings.AddPicture("pictureRight", logo2);
                pictureRight.SetPosition(_maxRow + 2, 0, _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT] - 1, 0);
                pictureRight.SetSize(113, 45);

                workSheet.Cells[_maxRow, 1, _maxRow + 5, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT]]
            ...
            else
            {
                int columnGWeight = _dicPositionColumnPackingLine[GROSS_WEIGHT];
                int columnMeasM3 = _dicPositionColumnPackingLine[MEAS_M3];
                //int columnPrePack = _dicPositionColumnPackingLine[PREPACK_TOTAL_QTY];

                if (_isAssortedSize_AssortedColor)
                {
                    workSheet.Cells[_maxRow, 4].Value = "Main Marks (both sides):";
                    workSheet.Cells[_maxRow, 4].Style.Font.Bold = true;
                    workSheet.Cells[_maxRow, 4, _maxRow, 6].Merge = true;
                    workSheet.Cells[_maxRow, columnGWeight].Value = "Side Marks (one side only):";
                    workSheet.Cells[_maxRow, columnGWeight].Style.Font.Bold = true;
                    workSheet.Cells[_maxRow, columnGWeight, _maxRow, columnGWeight + 4].Merge = true;
                }
                else
                {
                    workSheet.Cells[_maxRow, 4].Value = "(ALL MAIN MARK)";
                    workSheet.Cells[_maxRow, 5 + _countSize].Value = packinglist?.RatioLeft; /// Update 2022-10-27

                    workSheet.Cells[_maxRow, columnGWeight].Value = "(ONE SIZE MARK)";
                    workSheet.Cells[_maxRow, columnGWeight, _maxRow, columnGWeight + 3].Merge = true;
                    workSheet.Cells[_maxRow, columnGWeight +4].Value = packinglist?.RatioRight; /// Update 2022-10-27
                }

                //workSheet.Cells[_maxRow, columnPrePack].Value = packinglist?.RatioLeft; /// Update 2022-10-14
                //workSheet.Cells[_maxRow, columnMeasM3].Value = packinglist?.RatioRight; /// Update 2022-10-14


                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = packinglist.ShippingMark1Code;
                workSheet.Cells[_maxRow, columnGWeight].Value = "Customer CO. No:";
                workSheet.Cells[_maxRow, columnGWeight, _maxRow, columnGWeight + 3].Merge = true;
                workSheet.Cells[_maxRow, columnMeasM3].Value = _itemStyle.CustomerCodeNo;
                workSheet.Cells[_maxRow, columnMeasM3, _maxRow, columnMeasM3 + 2].Merge = true;

                if (_isAruni)
                {
                    _maxRow++;
                    workSheet.Cells[_maxRow, columnGWeight].Value = "Ultimate customer CO NO: ";
                    workSheet.Cells[_maxRow, columnGWeight, _maxRow, columnGWeight + 3].Merge = true;
                    workSheet.Cells[_maxRow, columnMeasM3].Value = _itemStyle.UCustomerCodeNo;
                    workSheet.Cells[_maxRow, columnMeasM3, _maxRow, columnMeasM3 + 2].Merge = true;
                }

                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = packinglist.ShippingMark2Code;
                workSheet.Cells[_maxRow, columnGWeight].Value = "Art. No:";
                workSheet.Cells[_maxRow, columnGWeight, _maxRow, columnGWeight + 3].Merge = true;
                workSheet.Cells[_maxRow, columnMeasM3].Value = _itemStyle.CustomerStyle + "-" +
                                                               _itemStyle.ColorCode;
                workSheet.Cells[_maxRow, columnMeasM3, _maxRow, columnMeasM3 + 2].Merge = true;
                ...
                workSheet.Cells[_maxRow, 5].Value = _itemStyle.CustomerCodeNo;
                workSheet.Cells[_maxRow, 5, _maxRow, columnGWeight - 1].Merge = true;
                workSheet.Cells[_maxRow, columnGWeight].Value = (_itemStyle?.CustomerCode ?? string.Empty).Trim().ToUpper() == "USPUA" ? "US Size" : "Size:"; /// Update 2022-09-27
                workSheet.Cells[_maxRow, columnGWeight, _maxRow, columnGWeight + 3].Merge = true;
                workSheet.Cells[_maxRow, columnMeasM3, _maxRow, columnMeasM3 + 2].Merge = true;
                ...
                {
                    _maxRow++;
                    workSheet.Cells[_maxRow, 4].Value = "Ultimate customer CO NO: ";
                    workSheet.Cells[_maxRow, 5].Value = _itemStyle.UCustomerCodeNo;
                ...
                workSheet.Cells[_maxRow, 5].Value = _itemStyle.CustomerStyle + "-" +
                                                    _itemStyle.ColorCode;
                workSheet.Cells[_maxRow, 5, _maxRow, columnGWeight - 1].Merge = true;
                workSheet.Cells[_maxRow, columnGWeight].Value = "QTY:";
                workSheet.Cells[_maxRow, columnGWeight, _maxRow, columnGWeight + 3].Merge = true;
                workSheet.Cells[_maxRow, columnMeasM3, _maxRow, columnMeasM3 + 2].Merge = true;
                ...
                if (_isAssortedSize_AssortedColor)
                {
                    workSheet.Cells[_maxRow, columnGWeight].Value = "GW:";
                    workSheet.Cells[_maxRow, columnGWeight + 4].Value = "KG";
                }
                else
                {
                    workSheet.Cells[_maxRow, columnGWeight].Value = "GW:";
                ...
                _maxRow++;
                workSheet.Cells[_maxRow, 4].Value = (_itemStyle?.CustomerCode ?? string.Empty).Trim().ToUpper() == "USPUA" ? "US Size" : "Size:"; /// Update 2022-09-27
                workSheet.Cells[_maxRow, 5, _maxRow, columnGWeight - 1].Merge = true;
                if (_isAssortedSize_AssortedColor)
                {
                    workSheet.Cells[_maxRow, columnGWeight].Value = "NW:";
                    workSheet.Cells[_maxRow, columnGWeight + 4].Value = "KG";
                }
                else
                {
                ...
                workSheet.Cells[_maxRow, 5, _maxRow, columnGWeight - 1].Merge = true;
                workSheet.Cells[_maxRow, columnGWeight].Value = "MEAS:";
                workSheet.Cells[_maxRow, columnGWeight, _maxRow, columnGWeight + 3].Merge = true;
                workSheet.Cells[_maxRow, columnMeasM3, _maxRow, columnMeasM3 + 2].Merge = true;
                ...
                Image logo1 = LogoDownload(shippingImage1);
                var pictureRight = workSheet.Drawings.AddPicture("pictureRight", logo1);
                pictureRight.SetPosition(_maxRow, 0, columnGWeight - 1, 0);

                workSheet.Cells[_maxRow + 5, columnGWeight].Value = packinglist.Company?.Name;
                workSheet.Cells[_maxRow + 5, columnGWeight].Style.Font.Italic = true;
                workSheet.Cells[_maxRow + 5, columnGWeight, _maxRow + 6, columnMeasM3 + 2]
                         .Merge = true;
                workSheet.Cells[_maxRow + 5, columnGWeight, _maxRow + 6, columnMeasM3 + 2]
                         .Style.WrapText = true;

                workSheet.Cells[_maxRow, 4].Value = "Carton No. Of";
                workSheet.Cells[_maxRow, 5, _maxRow, columnGWeight - 1].Merge = true;
                ...
                pictureLeft.SetPosition(_maxRow, 0, 0, 0);
                workSheet.Cells[_maxRow + 5, 1].Value = "(one main mark)";
                workSheet.Cells[_maxRow + 5, 4, _maxRow + 5, 8].Merge = true;

                //workSheet.Cells[_maxRow + 5, columnPrePack].Value = packinglist?.RatioBottom; /// Update 2022-10-14
                workSheet.Cells[_maxRow + 5, 5 + _countSize].Value = packinglist?.RatioBottom; /// Update 2022-10-27
            }
        }
        #endregion

        #region IFG
        private Stream CreateExcelFile_IFG(PackingList packingList, Stream stream = null)
        {
            string Author = "Leading Star VN";
        ...
        #region GARAN
        private Stream CreateExcelFile_GA(PackingList packingList, Stream stream = null)
        {
            string Author = "Leading Star VN";
        ...
        #region H&M 
        private Stream CreateExcelFile_HM(PackingList packingList, Stream stream = null)
        {
            string Author = "Leading Star VN";
            ...
            using (var excelPackage = new ExcelPackage(stream ?? new MemoryStream()))
            {
                ...
                CreateHeaderPage_HM(workSheet, packingList);
                CreateHeaderPackingLine_HM(workSheet, packingList);
                FillDataPackingLine_HM(workSheet, packingList);
                SummaryQuantitySize_Carton_HM(workSheet, packingList, shippingImage1);

                string modelRangeBorder = "A1:"
                                        + _dicAlphabel[_dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA]]
                                        + (_maxRow + 9);
                ...
                //string rangeSize = "G" + 7 + ":" + _dicAlphabel[_itemStyle.OrderDetails.Count + 6] + 7;
                //workSheet.Cells[rangeSize].Style.Fill.PatternType = ExcelFillStyle.Solid;
                //workSheet.Cells[rangeSize].Style.Fill.BackgroundColor.SetColor(Color.CornflowerBlue);

                if (_dicAlphabel.TryGetValue(_dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA], out string characterColumnPacking))
                {
                    using (var range = workSheet.Cells["A1:" + characterColumnPacking + "1"])
                    {
                        range.Merge = true;
        ...
        public void CreateHeaderPage_HM(ExcelWorksheet workSheet, PackingList packingList)
        {
            workSheet.Cells[1, 1].Value = "PACKING LIST";
            workSheet.Cells[1, 1].Style.Font.Bold = true;
            workSheet.Cells[1, 1].Style.Font.Color.SetColor(Color.Black);
            ...
            if(packingList.ItemStyles.Count > 1)
                _countSize = packingList.ItemStyles.Count;
            else if(packingList.PackingLines.FirstOrDefault().PrePack == "Assorted Size - Solid Color")
                _countSize = (int)(packingList.PackingLines.Select(x => x.QuantityPerPackage).FirstOrDefault());
            else
                _countSize = (int)(packingList.PackingLines.Select(x => x.Quantity).FirstOrDefault());
            ...
            workSheet.Cells[_maxRow, posCustomer].Style.Font.Bold = true;
            //workSheet.Cells[_maxRow, posCustomer, _maxRow, (posCustomer + 1)].Merge = true;
            workSheet.Cells[_maxRow, posCustomer + 1].Value = _itemStyle?.PurchaseOrderNumber ?? string.Empty;
            workSheet.Cells[_maxRow, posCustomer + 1].Style.Font.Bold = true;
            workSheet.Cells[_maxRow, posCustomer + 1].Style.HorizontalAlignment = ExcelHorizontalAlignment.Left;
            //workSheet.Cells[_maxRow, posCustomer + 2, _maxRow, (posCustomer + 5)].Merge = true;

            _maxRow++;
            workSheet.Cells[_maxRow, 1].Value = "Type:";
            workSheet.Cells["A" + _maxRow + ":C" + _maxRow].Merge = true;
            workSheet.Cells[_maxRow, 4].Value = _itemStyle?.ExternalPurchaseOrderTypeCode ?? string.Empty;

            _maxRow++;
            workSheet.Cells[_maxRow, 1].Value = "ORDER TYPE:";
            workSheet.Cells[_maxRow, 4].Value = _itemStyle?.ExternalPurchaseOrderTypeName ?? string.Empty;

            workSheet.Cells[_maxRow, posCustomer].Value = "PSDD:";
            workSheet.Cells[_maxRow, posCustomer].Style.Font.Bold = true;
            //workSheet.Cells[_maxRow, posCustomer, _maxRow, (posCustomer + 1)].Merge = true;
            workSheet.Cells[_maxRow, posCustomer + 1].Value = _itemStyle?.ProductionSkedDeliveryDate;
            workSheet.Cells[_maxRow, posCustomer + 1].Style.Numberformat.Format = "MM/dd/yyyy";
            workSheet.Cells[_maxRow, posCustomer + 1, _maxRow, (posCustomer + 2)].Merge = true;
            workSheet.Cells[_maxRow, posCustomer + 1].Style.HorizontalAlignment = ExcelHorizontalAlignment.Left;

            _maxRow += 3;
        }
        public void CreateHeaderPackingLine_HM(ExcelWorksheet workSheet, PackingList packinglist)
        {
            int column = 1;
            //int rowSize = _maxRow + 1;
            //int columnSize = 7;
            //_maxRow += 3;
            _dicPositionColumnPackingLine = new Dictionary<string, int>();

            /// Total quantity of size
            //foreach (var orderDetail in _itemStyle.OrderDetails.OrderBy(x => x.SizeSortIndex))
            //{
            //    workSheet.Cells[rowSize, columnSize].Value = orderDetail.Size;
            //    workSheet.Cells[rowSize, columnSize].Style.Font.Bold = true;
            //    workSheet.Cells[rowSize, columnSize].Style.WrapText = true;

            //    workSheet.Cells[rowSize + 1, columnSize].Value = packinglist.PackingLines
            //            .Where(x => x.Size == orderDetail.Size).Sum(x => x.QuantitySize * x.TotalCarton);
            //    workSheet.Cells[rowSize + 1, columnSize].Style.Numberformat.Format = "#,##0";

            //    columnSize++;
            //}

            //string range = "G" + rowSize + ":" + _dicAlphabel[_itemStyle.OrderDetails.Count + 6] + (rowSize + 1);
            ...
            range = "E" + _maxRow + ":E" + (_maxRow + 1);
            workSheet.Cells[_maxRow, column].Value = SIZE_HM;
            workSheet.Cells[_maxRow, column].Style.WrapText = true;
            workSheet.Cells[range].Merge = true;
            _dicPositionColumnPackingLine.Add(SIZE_HM, column);

            column++;

            //range = "F" + _maxRow + ":F" + (_maxRow + 1);
            //workSheet.Cells[_maxRow, column].Value = COLOR_NAME;
            //workSheet.Cells[_maxRow, column].Style.WrapText = true;
            //workSheet.Row(_maxRow).Height = 18;
            //workSheet.Cells[range].Merge = true;
            //_dicPositionColumnPackingLine.Add(COLOR_NAME, column);

            //column++;

            workSheet.Cells[_maxRow, column].Value = COLOR_NAME;
            workSheet.Cells[_maxRow, column].Style.Font.SetFromFont(new Font("Arial", 8));
            _dicPositionColumnPackingLine.Add(COLOR_NAME, column);
            var colorList = packinglist.ItemStyles.OrderBy(x => x.LSStyle).ToList();
            var firstColor = colorList[0];

            if (colorList.Count > 1)
            {
                var checkDiffColor = false;

                foreach (var color in colorList)
                {
                    workSheet.Cells[_maxRow + 1, column].Value = color.ColorName;
                    workSheet.Cells[_maxRow + 1, column].Style.WrapText = true;
                    _dicPositionColumnPackingLine.Add(color.LSStyle, column);

                    if (firstColor.ColorName != color.ColorName && !checkDiffColor)
                        checkDiffColor = true;
                ...
                if (_dicAlphabel.TryGetValue(column - 1, out string character))
                {
                    range = _dicAlphabel[_dicPositionColumnPackingLine[COLOR_NAME]] + _maxRow + ":" + character + _maxRow;
                    workSheet.Cells[range].Merge = true;

                    if (!checkDiffColor)
                    {
                        range = _dicAlphabel[_dicPositionColumnPackingLine[COLOR_NAME]] + (_maxRow + 1) + ":" + character + (_maxRow + 1);
                        workSheet.Cells[range].Merge = true;
                    }
                }
            ...
            {
                range = _dicAlphabel[_dicPositionColumnPackingLine[COLOR_NAME]] + _maxRow + ":" + 
                        _dicAlphabel[_dicPositionColumnPackingLine[COLOR_NAME] + (_countSize - 1)] + _maxRow;
                workSheet.Cells[range].Merge = true;

                workSheet.Cells[_maxRow + 1, column].Value = firstColor.ColorName;
                workSheet.Cells[_maxRow + 1, column].Style.WrapText = true;
                _dicPositionColumnPackingLine.Add(firstColor.LSStyle, column);

                range = _dicAlphabel[_dicPositionColumnPackingLine[COLOR_NAME]] + (_maxRow + 1) + ":" +
                        _dicAlphabel[_dicPositionColumnPackingLine[COLOR_NAME] + (_countSize - 1)] + (_maxRow + 1);
                workSheet.Cells[range].Merge = true;

                column += _countSize;

                if (_dicAlphabel.TryGetValue(column, out string unitsBundleCharacter))
                {
                    range = unitsBundleCharacter + _maxRow + ":" + unitsBundleCharacter + (_maxRow + 1);
                    workSheet.Cells[_maxRow, column].Value = UNITS_BUNDLE;
                    workSheet.Cells[range].Merge = true;
                    workSheet.Cells[range].Style.WrapText = true;
                    workSheet.Column(column).Width = 7;
                    _dicPositionColumnPackingLine.Add(UNITS_BUNDLE, column);
                }

                column++;

                if (_dicAlphabel.TryGetValue(column, out string bundleCartonCharacter))
                {
                    range = bundleCartonCharacter + _maxRow + ":" + bundleCartonCharacter + (_maxRow + 1);
                    workSheet.Cells[_maxRow, column].Value = BUNDLE_CARTON;
                    workSheet.Cells[range].Merge = true;
                    workSheet.Cells[range].Style.WrapText = true;
                    workSheet.Column(column).Width = 7;
                    _dicPositionColumnPackingLine.Add(BUNDLE_CARTON, column);
                }
            }
            ...
            {
                range = totalNetWeightCharacter + _maxRow + ":" + totalNetWeightCharacter + (_maxRow + 1);
                workSheet.Cells[_maxRow, column].Value = TOTAL_NET_WEIGHT_GA;
                workSheet.Cells[_maxRow, column].Style.WrapText = true;
                workSheet.Cells[range].Merge = true;
                workSheet.Column(column).Width = 8;
                _dicPositionColumnPackingLine.Add(TOTAL_NET_WEIGHT_GA, column);
            }
            ...
            {
                range = totalGrossWeightCharacter + _maxRow + ":" + totalGrossWeightCharacter + (_maxRow + 1);
                workSheet.Cells[_maxRow, column].Value = TOTAL_GROSS_WEIGHT_GA;
                workSheet.Cells[_maxRow, column].Style.WrapText = true;
                workSheet.Cells[range].Merge = true;
                _dicPositionColumnPackingLine.Add(TOTAL_GROSS_WEIGHT_GA, column);
        ...
        public void FillDataPackingLine_HM(ExcelWorksheet workSheet, PackingList packinglist)
        {
            string range = "A" + _maxRow + ":";
            decimal totalMeasM3 = 0;
            decimal measM3 = 0;
            ...
            var size = "";
            var ratio = "";
            for(var i = 0; i < _countSize; i++)
            {
                if(string.IsNullOrEmpty(size))
                {
                    size = sortPackingLines[i].Size;
                }
                else
                {
                    size += "/" + sortPackingLines[i].Size;
                ...
                {
                    ratio = Math.Round((decimal)sortPackingLines[i].Quantity,0).ToString();
                }
                else
                {
                    ratio += "-" + Math.Round((decimal)sortPackingLines[i].Quantity, 0);
                }
            }
            var sizeDisplay = size + " (" + ratio + ")";
            ...
            foreach (var packingLine in sortPackingLines)
            {
                if (packingLine.PrePack == "Assorted Size - Solid Color")
                {
                    if (fromNo != packingLine.FromNo && toNo != packingLine.ToNo)
                    {
                        _maxRow++;
                        colQty = 0;
                        ...
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[FROM_NO] + 1].Value = "~";
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TO_NO]].Value = packingLine.ToNo;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[COLOR_CODE]].Value = style?.ColorCode;
                        //workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[SIZE_HM]].Value = "Size";
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[SIZE_HM]].Value = sizeDisplay;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[SIZE_HM]].Style.Font.SetFromFont(new Font("Arial", 8));

                        //for (var i = 0; i < _countSize; i++)
                        //{
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[packingLine.LSStyle] + colQty].Value = packingLine.QuantitySize * packingLine.TotalCarton;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[packingLine.LSStyle] + colQty].Style.Font.SetFromFont(new Font("Arial", 8));
                         //}
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_BUNDLE]].Value = packingLine.QuantityPerPackage;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[BUNDLE_CARTON]].Value = packingLine.PackagesPerBox;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON]]
                                        .Value = (decimal)(packingLine.TotalQuantity / packingLine.TotalCarton);
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[NO_OF_CARTON]].Value = packingLine.TotalCarton;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_UNITS]].Value = packingLine.TotalQuantity;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[LENGTH]].Value = packingLine.Length;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[WIDTH]].Value = packingLine.Width;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[HEIGHT]].Value = packingLine.Height;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[MEAS_M3_GA]].Value = Math.Round(measM3, 3);
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT_GA]].Value
                                = Math.Round((decimal)packingLine.NetWeight, 3);
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA]].Value
                                = Math.Round((decimal)packingLine.GrossWeight, 3);

                        fromNo = (int)packingLine.FromNo;
                        toNo = (int)packingLine.ToNo;
                        workSheet.Row(_maxRow).Height = 20;

                        /// Total quantity over size
                        var key = packingLine.LSStyle + ";" + packingLine.Size;
                        if (_dicTotalQuantitySizeOverStyle.TryGetValue(key, out decimal quantity))
                        {
                        ...
                        {
                            _dicTotalQuantitySizeOverStyle[key] = (decimal)(packingLine.QuantitySize * packingLine.TotalCarton);
                        }

                        if (_dicTotalQuantitySize.TryGetValue(packingLine.Size, out decimal sizeQuantity))
                        {
                            sizeQuantity += (decimal)(packingLine.QuantitySize * packingLine.TotalCarton);
                            _dicTotalQuantitySize[packingLine.Size] = sizeQuantity;
                        }
                        else
                        {
                            _dicTotalQuantitySize[packingLine.Size] = (decimal)(packingLine.QuantitySize * packingLine.TotalCarton);
                        }
                    }
                    else
                    {
                ...
                //if (packingLine.PrePack == "Solid Size - Assorted Color")   
                else
                {
                    if ((fromNo != packingLine.FromNo && toNo != packingLine.ToNo) ||
                        (fromNo == packingLine.FromNo && toNo == packingLine.ToNo && currentSize != packingLine.Size))
                    {
                        _maxRow++;
                        ...
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[COLOR_CODE]].Value = style?.ColorCode;
                        //workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[SIZE_HM]].Value = "Size";
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[SIZE_HM]].Value = packingLine.Size;
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[SIZE_HM]].Style.Font.SetFromFont(new Font("Arial", 8));

                        if(packinglist.ItemStyles.Count > 1)
                        {
                            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[packingLine.LSStyle]].Value = packingLine.QuantitySize * packingLine.TotalCarton;
                            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[packingLine.LSStyle]].Style.Font.SetFromFont(new Font("Arial", 8));
                            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_POLYBAG]].Value = packingLine.QuantityPerPackage;
                            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[POLYBAGS_CARTON]].Value = packingLine.PackagesPerBox;
                        }
                        else
                        {
                            for(var i = 0; i < _countSize; i++)
                            {
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[packingLine.LSStyle] + i].Value = packingLine.PackagesPerBox * packingLine.TotalCarton;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[packingLine.LSStyle] + i].Style.Font.SetFromFont(new Font("Arial", 8));
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_BUNDLE]].Value = packingLine.QuantityPerPackage;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[BUNDLE_CARTON]].Value = packingLine.PackagesPerBox;
                            }
                        }
            ...
            for (int i = 0; i < mergeCell.Count; i++)
            {
                if (mergeCell[i] < mergeCell.Last())
                {
                    workSheet.Cells["A" + mergeCell[i] + ":" + "A" + (mergeCell[i + 1] - 1)].Merge = true;
                    workSheet.Cells["B" + mergeCell[i] + ":" + "B" + (mergeCell[i + 1] - 1)].Merge = true;
                    workSheet.Cells["C" + mergeCell[i] + ":" + "C" + (mergeCell[i + 1] - 1)].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[UNITS_CARTON],
                                   (mergeCell[i + 1] - 1), _dicPositionColumnPackingLine[UNITS_CARTON]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[NO_OF_CARTON],
                                   (mergeCell[i + 1] - 1), _dicPositionColumnPackingLine[NO_OF_CARTON]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[TOTAL_UNITS],
                                   (mergeCell[i + 1] - 1), _dicPositionColumnPackingLine[TOTAL_UNITS]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[LENGTH],
                                   (mergeCell[i + 1] - 1), _dicPositionColumnPackingLine[LENGTH]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[WIDTH],
                                   (mergeCell[i + 1] - 1), _dicPositionColumnPackingLine[WIDTH]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[HEIGHT],
                                   (mergeCell[i + 1] - 1), _dicPositionColumnPackingLine[HEIGHT]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[MEAS_M3_GA],
                                   (mergeCell[i + 1] - 1), _dicPositionColumnPackingLine[MEAS_M3_GA]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT_GA],
                                   (mergeCell[i + 1] - 1), _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT_GA]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA],
                                   (mergeCell[i + 1] - 1), _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA]].Merge = true;
                }
                else
                {
                    workSheet.Cells["A" + mergeCell[i] + ":" + "A" + (_maxRow - 1)].Merge = true;
                    workSheet.Cells["B" + mergeCell[i] + ":" + "B" + (_maxRow - 1)].Merge = true;
                    workSheet.Cells["C" + mergeCell[i] + ":" + "C" + (_maxRow - 1)].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[UNITS_CARTON],
                                   (_maxRow - 1), _dicPositionColumnPackingLine[UNITS_CARTON]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[NO_OF_CARTON],
                                   (_maxRow - 1), _dicPositionColumnPackingLine[NO_OF_CARTON]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[TOTAL_UNITS],
                                   (_maxRow - 1), _dicPositionColumnPackingLine[TOTAL_UNITS]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[LENGTH],
                                   (_maxRow - 1), _dicPositionColumnPackingLine[LENGTH]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[WIDTH],
                                   (_maxRow - 1), _dicPositionColumnPackingLine[WIDTH]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[HEIGHT],
                                   (_maxRow - 1), _dicPositionColumnPackingLine[HEIGHT]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[MEAS_M3_GA],
                                   (_maxRow - 1), _dicPositionColumnPackingLine[MEAS_M3_GA]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT_GA],
                                   (_maxRow - 1), _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT_GA]].Merge = true;
                    workSheet.Cells[mergeCell[i], _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA],
                                   (_maxRow - 1), _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA]].Merge = true;
                }
            }

            workSheet.Cells[styleRow, _dicPositionColumnPackingLine[COLOR_CODE],
                                   (_maxRow - 1), _dicPositionColumnPackingLine[COLOR_CODE]].Merge = true;
            //workSheet.Cells[styleRow, _dicPositionColumnPackingLine[SIZE],
            //               (_maxRow - 1), _dicPositionColumnPackingLine[SIZE]].Merge = true;

            var columnColorTotal = _dicPositionColumnPackingLine[COLOR_NAME];
            for(var i = 0; i < _countSize; i++)
            {
                workSheet.Cells[_maxRow, columnColorTotal + i].Value = _totalUnits/_countSize;
                workSheet.Cells[_maxRow, columnColorTotal + i].Style.Font.SetFromFont(new Font("Arial", 8));
            }

            _maxRow++;

            workSheet.Cells[_maxRow, 1,
                            _maxRow, _dicPositionColumnPackingLine[SIZE_HM]].Merge = true;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[COLOR_NAME]].Value = _itemStyle.Packing;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[COLOR_NAME]].Style.Font.Bold = true;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[COLOR_NAME],
                            _maxRow, packinglist.ItemStyles.Count > 1 ? _dicPositionColumnPackingLine[UNITS_POLYBAG] -1 : 
                            _dicPositionColumnPackingLine[UNITS_BUNDLE] -1].Merge = true;
            workSheet.Cells[_maxRow, packinglist.ItemStyles.Count > 1 ? _dicPositionColumnPackingLine[UNITS_POLYBAG] : 
                            _dicPositionColumnPackingLine[UNITS_BUNDLE], _maxRow, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA]].Merge = true;

            /// STYLE BORDER PACKING
            if (_dicAlphabel.TryGetValue(_dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA], out string maxColumnCharacter))
            {
                string rangePacking = "A" + _maxRow + ":" + maxColumnCharacter + _maxRow;
                /// BORDER ALL DATA
                using (var borderData = workSheet.Cells[rangePacking])
                {
                    borderData.Style.VerticalAlignment = ExcelVerticalAlignment.Center;
                    borderData.Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                    borderData.Style.Border.Top.Style = ExcelBorderStyle.Thin;
            ...
            _maxRow++;
            /// STYLE BORDER, FIT COLUMN ALL DATA
            range += maxColumnCharacter + (_maxRow - 1);
            using (var borderData = workSheet.Cells[range])
            {
                borderData.Style.VerticalAlignment = ExcelVerticalAlignment.Center;
                borderData.Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
            ...
            /// FORMAT NUMBER
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON] - 1].Value = _totalUnits / _countSize;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON] - 1].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

            range = _dicAlphabel[_dicPositionColumnPackingLine[COLOR_NAME]] + styleRow + ":" +
                    _dicAlphabel[_dicPositionColumnPackingLine[TOTAL_UNITS]] + (_maxRow + 3);
            workSheet.Cells[range].Style.Numberformat.Format = "#,##0";

            range = _dicAlphabel[_dicPositionColumnPackingLine[MEAS_M3_GA]] + styleRow + ":" +
                    _dicAlphabel[_dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA]] + (_maxRow + 3);
            workSheet.Cells[range].Style.Numberformat.Format = "#,##0.000";

            _maxRow++;
            // SET DATA TOTAL PACKING LINE 
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON] - 1].Value = "PPK";
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON] - 1].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
            ...
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[MEAS_M3_GA]].Value = Math.Round(totalMeasM3, 3);
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT_GA]].Value = _totalNetWeight;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA]].Value = _totalGrossWeight;
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON],
                            _maxRow, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA]]
                            .Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
            ...
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[NO_OF_CARTON]].Value = "Cnts";
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_UNITS]].Value = packinglist?.ItemStyles?.FirstOrDefault()?.UnitID ?? "PCS";
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_NET_WEIGHT_GA]].Value = "Kg";
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA]].Value = "Kg";
            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[UNITS_CARTON],
                            _maxRow, _dicPositionColumnPackingLine[TOTAL_GROSS_WEIGHT_GA]]
                            .Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
        ...
        public void SummaryQuantitySize_Carton_HM(ExcelWorksheet workSheet, PackingList packinglist, byte[] logoShipping)
        {
            /// Insert logo 
            //Image logo1 = LogoDownload(logoShipping);
            //var pictureRight = workSheet.Drawings.AddPicture("shippingImage1", logo1);
            //pictureRight.SetPosition((_maxRow - 1), 0, 2, 0);

            int columnSizeColor = 3;
            int columnUnit = _dicPositionColumnPackingLine[UNITS_CARTON];
            int i = 2;
            int totalRow = _maxRow;
            int columnTotal = 0;

            workSheet.Cells[_maxRow - 1, columnSizeColor].Value = "SUMMARY";
            workSheet.Cells[_maxRow - 1, columnSizeColor].Style.Font.Bold = true;

            workSheet.Cells[_maxRow, columnSizeColor].Value = COLOR_SIZE;
            workSheet.Cells[_maxRow, columnSizeColor, _maxRow, columnSizeColor + 1].Merge = true;
            foreach (var orderDetail in _itemStyle.OrderDetails.OrderBy(x => x.SizeSortIndex))
            {
                workSheet.Cells[_maxRow, (columnSizeColor + i)].Value = orderDetail.Size;
                workSheet.Cells[_maxRow, (columnSizeColor + i)].Style.WrapText = true;
                _dicPositionColumnPackingLine.Add(orderDetail.Size, (columnSizeColor + i));

                i++;
            }
            workSheet.Cells[_maxRow, (columnSizeColor + i)].Value = "Total";
            columnTotal = columnSizeColor + i;

            _maxRow++;

            workSheet.Cells[_maxRow, columnUnit].Value = "TOTAL NW:";
            workSheet.Cells[_maxRow, columnUnit, _maxRow, columnUnit + 1].Merge = true;
            workSheet.Cells[_maxRow, (columnUnit + 2)].Value = _totalNetWeight;
            workSheet.Cells[_maxRow, (columnUnit + 2)].Style.Numberformat.Format = "#,##0.000";

            workSheet.Cells[(_maxRow + 1), columnUnit].Value = "TOTAL GW:";
            workSheet.Cells[(_maxRow + 1), columnUnit, (_maxRow + 1), columnUnit + 1].Merge = true;
            workSheet.Cells[(_maxRow + 1), columnUnit].Style.VerticalAlignment = ExcelVerticalAlignment.Top;
            workSheet.Cells[(_maxRow + 1), (columnUnit + 2)].Value = _totalGrossWeight;
            workSheet.Cells[(_maxRow + 1), (columnUnit + 2)].Style.Numberformat.Format = "#,##0.000";
            workSheet.Cells[(_maxRow + 1), (columnUnit + 2)].Style.VerticalAlignment = ExcelVerticalAlignment.Top;

            int cartonRow = _maxRow + 3;
            decimal totalUnit = 0;
            foreach (var itemStyle in packinglist.ItemStyles.OrderBy(x => x.LSStyle))
            {
                if(packinglist.ItemStyles.Count > 1)
                {
                    totalUnit = 0;
                    workSheet.Cells[_maxRow, columnSizeColor].Value = itemStyle.ColorName;
                    workSheet.Cells[_maxRow, columnSizeColor, _maxRow, columnSizeColor + 1].Merge = true;
                    foreach (var data in _dicTotalQuantitySizeOverStyle)
                    {
                        if (data.Key.Split(";")[0] == itemStyle.LSStyle)
                        {
                            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[data.Key.Split(";")[1]]].Value = data.Value;
                            workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[data.Key.Split(";")[1]]]
                                    .Style.Font.SetFromFont(new Font("Arial", 8));
                            totalUnit += (decimal)data.Value;
                        }
                    }

                    workSheet.Cells[_maxRow, (columnSizeColor + i)].Value = totalUnit;

                    _maxRow++;
                }
                else
                {
                    for(var j = 0; j < _countSize; j++)
                    {
                        totalUnit = 0;
                        workSheet.Cells[_maxRow, columnSizeColor].Value = itemStyle.ColorName;
                        workSheet.Cells[_maxRow, columnSizeColor, _maxRow, columnSizeColor + 1].Merge = true;
                        foreach (var data in _dicTotalQuantitySizeOverStyle)
                        {
                            if (data.Key.Split(";")[0] == itemStyle.LSStyle)
                            {
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[data.Key.Split(";")[1]]].Value = data.Value / _countSize;
                                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[data.Key.Split(";")[1]]]
                                        .Style.Font.SetFromFont(new Font("Arial", 8));
                                totalUnit += (decimal)data.Value / _countSize;
                            }
                        }
            ...
            workSheet.Cells[_maxRow, columnSizeColor].Value = "Total";
            workSheet.Cells[_maxRow, columnSizeColor, _maxRow, columnSizeColor + 1].Merge = true;
            totalUnit = 0;
            ...
            string range = _dicAlphabel[(columnSizeColor + 1)] + (totalRow + 1) + ":" +
                            _dicAlphabel[(columnSizeColor + i)] + _maxRow;
            workSheet.Cells[range].Style.Numberformat.Format = "#,##0";

            /// BORDER TOTAL QUANTITY
            string rangeBoder = _dicAlphabel[columnSizeColor] + totalRow + ":" +
                            _dicAlphabel[(columnSizeColor + i)] + _maxRow;
            using (var borderData = workSheet.Cells[rangeBoder])
            {
                borderData.Style.VerticalAlignment = ExcelVerticalAlignment.Center;
                ...
                borderData.Style.Border.Left.Style = ExcelBorderStyle.Thin;
                borderData.Style.WrapText = true;
                borderData.Style.Font.SetFromFont(new Font("Arial", 8));
            }

            _maxRow += 2;

            /// ORDER - SHIP - BALANCE - % SHORT SHIP QUANTITY
            var styleRow = _maxRow;
            var summaryOrderDetails = new Dictionary<string, decimal>();
            var shipQuantities = new Dictionary<string, decimal>();
            var shortShipQuantities = new Dictionary<string, decimal>();
            var firstStyle = packinglist.ItemStyles.FirstOrDefault();
            decimal totalAll = 0;

            firstStyle.OrderDetails.ToList().ForEach(x =>
            {
                summaryOrderDetails.Add(x.Size, x.ShipQuantity ?? 0);
                shipQuantities.Add(x.Size, 0);
                totalAll += (decimal)x.ShipQuantity;
            });

            packinglist.ItemStyles.Where(i => i.Number != firstStyle.Number).ToList().ForEach(i =>
            {
                i.OrderDetails.ToList().ForEach(o =>
                {
                    summaryOrderDetails[o.Size] += o.ShipQuantity ?? 0;
                    totalAll += (decimal)o.ShipQuantity;
                });
            });

            workSheet.Cells[_maxRow, columnSizeColor].Value = "ORDER";
            workSheet.Cells[_maxRow, columnSizeColor, _maxRow, columnSizeColor + 1].Merge = true;
            totalUnit = 0;
            foreach (var data in summaryOrderDetails)
            {
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[data.Key]].Value = data.Value;
                totalUnit += data.Value;
            }
            workSheet.Cells[_maxRow, columnTotal].Value = totalUnit;

            if (firstStyle.MultiShip == true)
            {
                decimal oldPackQty = 0;
                for (var j = 1; j < packinglist.OrdinalShip; j++)
                {
                    totalUnit = 0;
                    var oldPackingLines = firstStyle.PackingLists.Where(p => p.OrdinalShip == j)
                        .OrderByDescending(p => p.PackingListDate).FirstOrDefault().PackingLines.ToList();

                    var ordinalShip = j.ToString() + (j == 1 ? "st" : (j == 2 ? "nd" : (j == 3 ? "rd" : "th")));
                    _maxRow++;
                    workSheet.Cells[_maxRow, columnSizeColor].Value = ordinalShip + " SHIP";
                    workSheet.Cells[_maxRow, columnSizeColor, _maxRow, columnSizeColor + 1].Merge = true;
                    firstStyle.OrderDetails.ToList().ForEach(x =>
                    {
                        oldPackQty = (decimal)oldPackingLines.Where(p => p.Size == x.Size).Sum(p => p.QuantitySize * p.TotalCarton);
                        workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[x.Size]].Value = oldPackQty;
                        totalUnit += oldPackQty;
                        shipQuantities[x.Size] += oldPackQty;
                    });
            ...
            workSheet.Cells[_maxRow, columnSizeColor].Value = "SHIP";
            workSheet.Cells[_maxRow, columnSizeColor, _maxRow, columnSizeColor + 1].Merge = true;
            totalUnit = 0;
            foreach (var data in _dicTotalQuantitySize)
            {
            ...
            totalUnit = 0;
            workSheet.Cells[_maxRow, columnSizeColor].Value = "BALANCE";
            workSheet.Cells[_maxRow, columnSizeColor, _maxRow, columnSizeColor + 1].Merge = true;
            totalUnit = 0;
            foreach (var data in _dicTotalQuantitySize)
            {
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[data.Key]]
                        .Value = summaryOrderDetails[data.Key] - shipQuantities[data.Key] - data.Value;
                shortShipQuantities.Add(data.Key, Math.Round((summaryOrderDetails[data.Key] 
                        - shipQuantities[data.Key] - data.Value) * 100/ summaryOrderDetails[data.Key],2));
                totalUnit += summaryOrderDetails[data.Key] - shipQuantities[data.Key] - data.Value;
            }
            ...
            workSheet.Cells[_maxRow, columnSizeColor].Value = "% SHORT SHIP";
            workSheet.Cells[_maxRow, columnSizeColor, _maxRow, columnSizeColor + 1].Merge = true;
            foreach (var data in shortShipQuantities)
            {
                workSheet.Cells[_maxRow, _dicPositionColumnPackingLine[data.Key]].Value = data.Value + "%";
            }
            workSheet.Cells[_maxRow, columnTotal].Value = Math.Round(totalUnit * 100 / totalAll, 2) + "%";

            range = _dicAlphabel[columnSizeColor] + styleRow + ":" +
                            _dicAlphabel[columnTotal] + _maxRow;
            workSheet.Cells[range].Style.Numberformat.Format = "#,##0";
            ...
            /// CARTON SUMMARY
            int maxCartonRow = cartonRow;
            workSheet.Cells[maxCartonRow, columnUnit].Value = "CTNS SUMMARY";
            workSheet.Cells[maxCartonRow, columnUnit, maxCartonRow, columnUnit + 1].Merge = true;
            workSheet.Cells[maxCartonRow, (columnUnit + 2)].Value = "QTY";
            ...
            foreach (var carton in _dicTotalCartons)
            {
                workSheet.Cells[maxCartonRow, columnUnit].Value = carton.Key;
                workSheet.Cells[maxCartonRow, columnUnit, maxCartonRow, columnUnit + 1].Merge = true;
                workSheet.Cells[maxCartonRow, columnUnit + 2].Value = (decimal)carton.Value;
                totaCarton += (decimal)carton.Value;
            ...
            workSheet.Cells[maxCartonRow, columnUnit, maxCartonRow, columnUnit + 1].Merge = true;
            workSheet.Cells[maxCartonRow, columnUnit + 2].Value = totaCarton;
            workSheet.Cells[maxCartonRow, columnUnit + 2].Style.Font.Bold = true;

            /// FORMAT NUMBER
            range = _dicAlphabel[columnUnit + 2] + (cartonRow + 1) + ":" +
                            _dicAlphabel[columnUnit + 2] + maxCartonRow;
            workSheet.Cells[range].Style.Numberformat.Format = "#,##0";

            /// BORDER CTNS SUMMARY
            rangeBoder = _dicAlphabel[columnUnit] + cartonRow + ":" +
                                _dicAlphabel[columnUnit + 2] + maxCartonRow;
            using (var borderData = workSheet.Cells[rangeBoder])
            {
                borderData.Style.VerticalAlignment = ExcelVerticalAlignment.Center;
            ...
            workSheet.Cells[_maxRow, columnUnit]
                .Value = packinglist.Company != null ? packinglist.Company.Name.Trim().ToUpper()
                         : "LEADING STAR VIETNAM GARMENT COMPANY LIMITED";
            workSheet.Cells[_maxRow, columnUnit].Style.Font.Bold = true;
            workSheet.Cells[_maxRow, columnUnit].Style.Font.Italic = true;
        }
        #endregion

        #region HADDAD
        private Stream CreateExcelFile_HA(PackingList packingList, Stream stream = null)
        {
            string Author = "Leading Star VN";
        ...
        #region DECATHLON
        private Stream CreateExcelFile_DE(PackingList packingList, Stream stream = null)
        {
            //DataTable table = SetData(packingList);
