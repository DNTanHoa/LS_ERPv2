...
{
    public class CountPackingList_Win_ViewController
         : ObjectViewController<ListView, PackingList>
    {
        public CountPackingList_Win_ViewController()
        {
        ...
        private void gridview_CustomCount(object sender, CustomSummaryEventArgs e)
        {
            ...
            List<PackingList> rows = new List<PackingList>();
            ...
            for (int i = 0; i < selectedRows.Length; i++)
            {
                ...
                {
                    var row = gridView.GetRow(selectedRowHandle) as PackingList;
                    if (row != null)
                        rows.Add(row);
                }
            }
