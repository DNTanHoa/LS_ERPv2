...
{
    public partial class SummaryProductionBomGrid_ViewController 
        : ObjectViewController<ListView, ProductionBOM>
    {
        public SummaryProductionBomGrid_ViewController()
        {
            InitializeComponent();
            TargetObjectType = typeof(ProductionBOM);
        ...
        private void gridview_CustomSummaryCalculate(object sender, CustomSummaryEventArgs e)
        {
            ...
            List<ProductionBOM> rows = new List<ProductionBOM>();
            ...
            for (int i = 0; i < selectedRows.Length; i++)
            {
                ...
                {
                    var row = gridView.GetRow(selectedRowHandle) as ProductionBOM;
                    if (row != null)
                        rows.Add(row);
                }
            }
            ...
            try
            {
                if (rows != null & rows.Count > 0)
                {
                    ...
                    {
                        var totalEntryQuantity = rows.Sum(x => x.WareHouseQuantity);
