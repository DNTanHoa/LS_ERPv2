...
namespace LS_ERP.API.Controllers
{
    ...
    [ApiController]
    public class LabelPortController : ControllerBase
    {
        private readonly ILogger<LabelPortController> logger;
        private readonly IMediator mediator;

        public LabelPortController(ILogger<LabelPortController> logger,
            IMediator mediator)
        {
            this.logger = logger;
        ...
        public async Task<ActionResult<CommonResponseModel>> Import(
            [FromForm] ImportLabelPortCommand command)
        {
            var commandResult = new ImportLabelPortResult();
            logger.LogInformation("{@time} - Sending import Label Port command with request {@request}",
